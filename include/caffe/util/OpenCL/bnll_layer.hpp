...
namespace OpenCL {
	...
	template<typename T> bool clBNLLLayerForward(const int count, const T* bottom_data, T* top_data);
	template<typename T> bool clBNLLLayerBackward(const int count, const T* top_diff, const T* bottom_data, T* bottom_diff);
} // namespace OpenCL
