...
/**
 * @brief A weighted loss that takes a
 *        "weight" (distance) matrix specifying the cost of all label
 *        pairs: that is, the cost of assigning label i for GT label j.
 *
 *        Useful for assigning weight per label for tackling 
 *        label imbalance issues, etc.
 *
 *        (\b This layer expect class probabilities (like Infogain)),
 *        thus, it is recommanded to give it as "bottom" the "top" of
 *        a Softmax layer.
 *
 * ( Equivalent to the SoftmaxLoss when weights are 1 - identity).
 *
 * @param bottom input Blob vector (length 2-3)
 *   -# @f$ (N \times C \times H \times W) @f$
 *      the predictions @f$ \hat{p} @f$, a Blob with values in
 ...
 *      should sum to 1 as in a probability distribution: @f$
 *      \forall n \sum\limits_{k=1}^K \hat{p}_{nk} = 1 @f$.
 *      (should accept as input the output of a SOFTMAX layer (?))
 *   -# @f$ (N \times 1 \times 1 \times 1) @f$
 *      the labels @f$ l @f$, an integer-valued Blob with values
 *      @f$ l_n \in [0, 1, 2, ..., K - 1] @f$
 *      indicating the correct class label among the @f$ K @f$ classes
 *   -# @f$ (1 \times 1 \times K \times K) @f$
 *      (\b optional) the weights matrix @f$ W @f$.  This must be provided as
 *      the third bottom blob input if not provided as the weighted_mat in the
 *      InfogainLossParameter. If @f$ W = 1 - I @f$, this layer is equivalent to the
 *      SOFTMAX_LOSS layer.
 * @param top output Blob vector (length 1)
 *   -# @f$ (1 \times 1 \times 1 \times 1) @f$
 *      the computed weighted loss: @f$ E =
 *        \frac{1}{N} \sum\limits_{n=1}^N W_{l_n} \hat{p}_n =
 *        \frac{1}{N} \sum\limits_{n=1}^N \sum\limits_{k=1}^{K} W_{l_n,k}
 *        \hat{p}_{n,k}
 *      @f$, where @f$ W_{l_n} @f$ denotes row @f$l_n@f$ of @f$W@f$.
 */
template <typename Dtype>
class WeightedLossLayer : public LossLayer<Dtype> {
 public:
  explicit WeightedLossLayer(const LayerParameter& param)
      : LossLayer<Dtype>(param), weights_() {}
  virtual void LayerSetUp(const vector<Blob<Dtype>*>& bottom,
      const vector<Blob<Dtype>*>& top);
  virtual void Reshape(const vector<Blob<Dtype>*>& bottom,
      const vector<Blob<Dtype>*>& top);

  // WeightedLossLayer takes 2-3 bottom Blobs; if there are 3 the third should
  // be the weights matrix.  (Otherwise the weights matrix is loaded from a
  // file specified by LayerParameter.)
  virtual inline int ExactNumBottomBlobs() const { return -1; }
  virtual inline int MinBottomBlobs() const { return 2; }
  virtual inline int MaxBottomBlobs() const { return 3; }

  virtual inline const char* type() const { return "WeightedLoss"; }

 protected:
  /// @copydoc WeightedLossLayer
  virtual void Forward_cpu(const vector<Blob<Dtype>*>& bottom,
      const vector<Blob<Dtype>*>& top);

  /**
   * @brief Computes the weighted loss error gradient w.r.t. the predictions.
   *
   * Gradients cannot be computed with respect to the label inputs (bottom[1]),
   * so this method ignores bottom[1] and requires !propagate_down[1], crashing
   * if propagate_down[1] is set. (The same applies to the weights matrix, if
   * provided as bottom[2] rather than in the layer_param.)
   *
   * @param top output Blob vector (length 1), providing the error gradient
   *      with respect to the outputs
