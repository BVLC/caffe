...
public:
    explicit TripletLossLayer(const LayerParameter& param)
        : LossLayer<Dtype>(param), diff_a_n_(), loss_(), diff_a_p_(), diff_n_p_(), diff_p_a_() { }
    virtual void LayerSetUp(const vector<Blob<Dtype>*>& bottom,
                            const vector<Blob<Dtype>*>& top);
...
protected:
    ...
    Blob<Dtype> diff_a_n_;    // cached for backward pass
    Blob<Dtype> loss_;        // cached for backward pass
    Blob<Dtype> diff_a_p_;    // actually does not need to be cached
    Blob<Dtype> diff_n_p_;    // actually does not need to be cached
    Blob<Dtype> diff_p_a_;    // actually does not need to be cached
    // gpu
    Blob<Dtype> diff_sq_a_p_; // tmp storage for gpu forward pass
    Blob<Dtype> diff_sq_a_n_; // tmp storage for gpu forward pass
    Blob<Dtype> dist_sq_a_p_; // tmp storage for gpu forward pass
    Blob<Dtype> dist_sq_a_n_; // tmp storage for gpu forward pass
    Blob<Dtype> summer_vec_;  // tmp storage for gpu forward pass
};
