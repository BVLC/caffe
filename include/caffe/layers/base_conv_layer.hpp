...
class BaseConvolutionLayer : public Layer<Dtype, MItype, MOtype> {
 ...
 protected:
  ...
  void forward_gpu_gemm(vptr<const Dtype> col_input, const uint_tp col_input_off,
                        vptr<const Dtype> weights, vptr<const Dtype> output,
 ...
 private:
...
#ifndef CPU_ONLY
  inline void conv_im2col_gpu(vptr<Dtype> data, vptr<Dtype> col_buff) {
    if (!force_nd_im2col_ && num_spatial_axes_ == 2) {
      this->device_->im2col_gpu(data, conv_in_channels_,
          conv_input_shape_.cpu_data()[1], conv_input_shape_.cpu_data()[2],
          kernel_shape_.cpu_data()[0], kernel_shape_.cpu_data()[1],
          pad_.cpu_data()[0], pad_.cpu_data()[1],
          stride_.cpu_data()[0], stride_.cpu_data()[1],
      ...
      this->device_->im2col_nd_gpu(data, num_spatial_axes_, num_kernels_im2col_,
  ...
  inline void conv_col2im_gpu(vptr<Dtype> col_buff, vptr<Dtype> data) {
    if (!force_nd_im2col_ && num_spatial_axes_ == 2) {
      this->device_->col2im_gpu(col_buff, conv_in_channels_,
          conv_input_shape_.cpu_data()[1], conv_input_shape_.cpu_data()[2],
          kernel_shape_.cpu_data()[0], kernel_shape_.cpu_data()[1],
          pad_.cpu_data()[0], pad_.cpu_data()[1],
          stride_.cpu_data()[0], stride_.cpu_data()[1],
      ...
      this->device_->col2im_nd_gpu(col_buff, num_spatial_axes_,
