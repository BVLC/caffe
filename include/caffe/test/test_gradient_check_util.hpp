...
class GradientChecker {
 public:
    ...
    // computation only (e.g., neuron layers) -- where (d y_i) / (d x_j) = 0
    // when i != j.
    void CheckGradientEltwise(
        Layer<Dtype>* layer,
        const vector<Blob<Dtype>*>& bottom,
        const vector<Blob<Dtype>*>& top);

    void CheckGradientSingle(
        Layer<Dtype>* layer,
        const vector<Blob<Dtype>*>& bottom,
        const vector<Blob<Dtype>*>& top,
        int check_bottom,
    ...
    void CheckGradientNet(
        const Net<Dtype>& net,
        const vector<Blob<Dtype>*>& input);

 protected:
    Dtype GetObjAndGradient(
        const Layer<Dtype>& layer,
        const vector<Blob<Dtype>*>& top,
        int top_id = -1,
        int top_data_id = -1);
    Dtype stepsize_;
...
void GradientChecker<Dtype>::CheckGradientSingle(
  ...
  for (int blob_id = 0; blob_id < blobs_to_check.size(); ++blob_id) {
    ...
    computed_gradient_blobs[blob_id].reset(
        new Blob<Dtype>());
    ...
    for (int feat_id = 0; feat_id < current_blob->count(); ++feat_id) {
      ...
      if (!element_wise || (feat_id == top_data_id)) {
        ...
        layer->Forward(
            bottom,
            top);
        positive_objective = GetObjAndGradient(
            *layer,
            top,
            top_id,
            top_data_id);
        ...
        layer->Forward(
            bottom,
            top);
        negative_objective = GetObjAndGradient(
            *layer,
            top,
            top_id,
            top_data_id);
