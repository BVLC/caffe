...
void GradientChecker<Dtype>::CheckGradientSingle(Layer<Dtype>* layer,
  ...
  for (int blob_id = 0; blob_id < blobs_to_check.size(); ++blob_id) {
    ...
    Dtype deviation = 0;
    Dtype max_deviation = 0;
    Dtype min_deviation = 0;
    Dtype max_scale = 0;
    int dim = 1;
    if (current_blob->num_axes() > 0) {
      dim = current_blob->count() / current_blob->shape(0);
    }
...
template <typename Dtype>
void GradientChecker<Dtype>::CheckGradientUpToShift(Layer<Dtype>* layer,
    const vector<Blob<Dtype>*>& bottom, const vector<Blob<Dtype>*>& top,
    int check_bottom) {
  layer->SetUp(bottom, top);
  CHECK_GT(top.size(), 0) << "Exhaustive mode requires at least one top blob.";
  const bool element_wise = false;
  const bool up_to_shift = true;
  // LOG(ERROR) << "Up to shift.";
  for (int i = 0; i < top.size(); ++i) {
