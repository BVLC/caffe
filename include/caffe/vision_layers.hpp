...
class BaseConvolutionLayer: public Layer<Dtype> {
 ...
 protected:
    ...
    // The last argument in forward_cpu_gemm is so that we can skip the im2col
    // if we just called weight_cpu_gemm with the same input.
    ...
    inline void conv_im2col_gpu_quick(
        const Dtype* data,
        const size_t data_offset,
        Dtype* col_buff,
        const size_t col_buff_offset) {
      DLOG(INFO)<< "conv_im2col_gpu_quick called.";
      im2col_group_gpu(data, this->im2col_mask_.gpu_data(),
                       num_, channels_, height_, width_,
                       kernel_h_, kernel_w_,
                       height_out_, width_out_, col_buff);
    }
    inline void conv_im2col_gpu_all(const Dtype* data,
                                    const size_t data_offset,
                                    Dtype* col_buff,
                                    const size_t col_buff_offset) {
      im2col_gpu(data, data_offset,
                 channels_, height_, width_,
                 kernel_h_, kernel_w_, pad_h_, pad_w_, stride_h_, stride_w_,
                 col_buff, col_buff_offset);
    }

    inline void conv_col2im_gpu(const Dtype* col_buff,
                                const size_t col_buff_offset,
                                Dtype* data, const size_t data_offset) {
      col2im_gpu(col_buff, col_buff_offset,
                 conv_in_channels_, conv_in_height_, conv_in_width_,
                 kernel_h_, kernel_w_, pad_h_, pad_w_, stride_h_, stride_w_,
                 data, data_offset);
    }
#endif
 ...
 private:
   ...
   *   high-throughput and generality of input and filter dimensions but comes
   *   at the cost of memory for matrices. This makes use of efficiency in BLAS.
   *
...
class ConvolutionLayer: public BaseConvolutionLayer<Dtype> {
 public:
     ...
     *  stride, given by stride_size for equal dimensions or stride_h and
     *  stride_w for different strides. By default the convolution is dense
     *  with stride 1.
     ...
     *  dimensions must be divisible by the number of groups.
     *  For group @f$ \geq 1 @f$, the
     *  convolutional filters' input and output channels are separated s.t. each
     *  group takes 1 / group of the input channels and makes 1 / group of the
     *  output channels. Concretely 4 input channels, 8 output channels, and
     *  2 groups separate input channels 1-2 and output channels 1-4 into the
     *  first group and input channels 3-4 and output channels 5-8 into the
     *  second group.
     *  - bias_term (\b optional, default true). Whether to have a bias.
     *  - engine: convolution has CAFFE (matrix multiplication) and CUDNN
     *  (library kernels + stream parallelism) engines.
     */
...
class LRNLayer: public Layer<Dtype> {
 ...
 protected:
    ...
    virtual void CrossChannelForward_gpu(
        const vector<Blob<Dtype>*>& bottom,
        const vector<Blob<Dtype>*>& top);
    virtual void WithinChannelForward(
        const vector<Blob<Dtype>*>& bottom,
        const vector<Blob<Dtype>*>& top);
    virtual void CrossChannelBackward_cpu(
        const vector<Blob<Dtype>*>& top,
        const vector<bool>& propagate_down,
        const vector<Blob<Dtype>*>& bottom);
    virtual void CrossChannelBackward_gpu(
        const vector<Blob<Dtype>*>& top,
        const vector<bool>& propagate_down,
        const vector<Blob<Dtype>*>& bottom);
    virtual void WithinChannelBackward(
        const vector<Blob<Dtype>*>& top,
        const vector<bool>& propagate_down,
        const vector<Blob<Dtype>*>& bottom);
