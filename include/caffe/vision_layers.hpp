...
class Convolution3DLayer : public Layer<Dtype> {
 public:
  ...
  inline int MinTopBlobs() const { return 1; }
  inline bool EqualNumBottomTopBlobs() const { return true; }

  inline const char* type() const { return "Convolution3D"; }
 ...
 protected:
  ...
  // reverse_dimensions should return true iff we are implementing deconv, so
  // that conv helpers know which dimensions are which.
  inline bool reverse_dimensions() { return false; }
  // Compute height_out_ and width_out_ and depth_out_ from other parameters.
  ...
  int num_;
  int channels_;
  int pad_h_, pad_w_, pad_d_;
  int height_, width_, depth_;
  int group_;
  int num_output_;
  int height_out_, width_out_, depth_out_;
  bool bias_term_;
  bool is_1x1_;
 ...
 private:
  ...
  inline void conv_vol2col_cpu(const Dtype* data, Dtype* col_buff) {
    vol2col_cpu(data, conv_in_channels_, conv_in_height_, conv_in_width_,
