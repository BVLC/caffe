...
struct DefaultSolverActions
{
    ...
    DefaultSolverActions( const SolverParameter& solver_param ):
        param_( solver_param )
    {
    }
    ...
    IterActions<Dtype> operator()( const TrainingStats<Dtype>& stats )
    {
        ...
        actions = stats.GetIter() < param_.max_iter() ?
        ...
        {
            actions = actions.SetShouldDisplay();
        }

        actions = actions.SetLearnRate( this->GetLearningRate( stats ) );
        actions = actions.SetMomentum( this->param_.momentum() );
        actions = actions.SetWeightDecay( this->param_.weight_decay() );
        ...
        if ( stats.GetIter() == 0 )
        {
            ...
            {
                actions = actions.SetResumeFile( resume_file_ );
            }
        ...
        // We have begun training.
        {
            ...
            if ( param_.snapshot() && ( stats.GetIter() %
                                        param_.snapshot() == 0 ) )
            {
                actions = actions.SetShouldSnapshot();
            }
        }
        return actions;
    }
    ...
    Dtype GetLearningRate( const TrainingStats<Dtype>& stats ) {
      ...
      if (lr_policy == "fixed") {
        ...
        rate = this->param_.base_lr() * pow(this->param_.gamma(), stats.GetIter() );
