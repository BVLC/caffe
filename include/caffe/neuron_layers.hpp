...
/**
 * @brief Guided Backprop Rectified Linear Unit non-linearity @f$ y = \max(0, x) @f$.
 *        The simple max is fast to compute, and the function does not saturate.
 */
template <typename Dtype>
class GBReLULayer : public NeuronLayer<Dtype> {
 public:
  /**
   * @param param provides ReLUParameter relu_param,
   *     with ReLULayer options:
   ...
   *     the value @f$ \nu @f$ by which negative values are multiplied.
   */
  explicit GBReLULayer(const LayerParameter& param)
      : NeuronLayer<Dtype>(param) {}

  virtual inline const char* type() const { return "GBReLU"; }
 ...
 */
template <typename Dtype>
class CuDNNGBReLULayer : public GBReLULayer<Dtype> {
 public:
  explicit CuDNNGBReLULayer(const LayerParameter& param)
      : GBReLULayer<Dtype>(param), handles_setup_(false) {}
  virtual void LayerSetUp(const vector<Blob<Dtype>*>& bottom,
      const vector<Blob<Dtype>*>& top);
  virtual void Reshape(const vector<Blob<Dtype>*>& bottom,
      const vector<Blob<Dtype>*>& top);
  virtual ~CuDNNGBReLULayer();
