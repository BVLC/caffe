...
void PyNet::AddDatumVector(bp::object data_obj, bp::object labels_obj)
{
    ...
    {
        vec[idx].set_label((int)(static_cast<float*>(PyArray_DATA(labels_arr))[idx]));
        for (size_t fidx = 0; fidx < md_layer->channels() * md_layer->height() * md_layer->width(); fidx++)
        {
            vec[idx].add_float_data((static_cast<float*>(PyArray_DATA(data_arr))
                          [idx * md_layer->channels() * md_layer->height() * md_layer->width() + fidx]));
        }
        vec[idx].set_channels(md_layer->channels());
        vec[idx].set_height(md_layer->height());
        vec[idx].set_width(md_layer->width());
    }
