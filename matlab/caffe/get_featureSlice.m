...
% @param bbox.{x1 x2 y1 y2} -- in image coordinates.
...
function featureSlice = get_featureSlice(pyra, bbox, templateSize, imageSize)
...
%pyra.sbin = 8;
  ...
  %1. select a scale in the pyramid
    ...
    bbox_desc = bbox_mult(bbox, 1.0/single(pyra.sbin)) % unscaled image -> unscaled feature coords.
  ...
  %2. rip out a slice from the appropriate scale
    ...
    bbox_to_use.x1 = bbox_desc.x1 * scale_to_use; %TODO: add padx/sbin
    bbox_to_use.x2 = bbox_to_use.x1 + templateSize(2);
    bbox_to_use.y1 = bbox_desc.y1 * scale_to_use; %TODO: add pady/sbin
    bbox_to_use.y2 = bbox_to_use.y1 + templateSize(1);
    bbox_to_use
    scale_to_use

    %TODO: add padx/sbin, pady/sbin
