#!/bin/bash
# Usage parse_log_googlenet.sh caffe.log
...
if [ "$#" -lt 1 ]
then
echo "Usage parse_log_googlenet.sh /path/to/your.log"
exit
fi
LOG=`basename $1`
grep -B 1 'Test ' $1 > aux.txt
grep 'Iteration ' aux.txt | sed  's/.*Iteration \([[:digit:]]*\).*/\1/g' > aux0.txt
#grep 'Test net output #0' aux.txt | awk '{print $11}' > aux1.txt # accuracy
#grep 'Test net output #1' aux.txt | awk '{print $11}' > aux2.txt # loss
grep 'Test net output #5' aux.txt | awk '{print $11}' > aux1.txt # accuracy
grep 'Test net output #4' aux.txt | awk '{print $11}' > aux2.txt # loss
