  ...
  CUDA_KERNEL_LOOP(index, n) {
    ...
    for (int i = 0; i < kernel_h; ++i) {
      for (int j = 0; j < kernel_w; ++j) {
        int h_im = h_offset + i * kernel_stride_h;
        int w_im = w_offset + j * kernel_stride_w;
        *data_col_ptr =
            (h_im >= 0 && w_im >= 0 && h_im < height && w_im < width) ?
            data_im_ptr[(i * kernel_stride_h) * width + j * kernel_stride_w]
            : 0;
...
void im2col_gpu(const Dtype* data_im, const int channels,
  ...
  im2col_gpu_kernel<Dtype><<<CAFFE_GET_BLOCKS(num_kernels),
      ...
      stride_h, stride_w, kernel_stride_h, kernel_stride_w,
...
__global__ void im2col_nd_gpu_kernel(const int n, const Dtype* data_im,
  ...
  CUDA_KERNEL_LOOP(index, n) {
    ...
    do {
      bool in_range = true;
      for (i = 0; i < num_axes; ++i) {
        const int d_iter_im = d_iter[i] * kernel_stride[i] + d_temp[i];
        in_range &= d_iter_im >= 0 && d_iter_im < im_shape[i + 1];
        if (!in_range) { break; }
      }
      if (in_range) {
        int data_im_offset = d_iter[0] * kernel_stride[0];
        for (i = 1; i < num_axes; ++i) {
          data_im_offset *= im_shape[i + 1];
          data_im_offset += d_iter[i] * kernel_stride[i];
        }
...
__global__ void col2im_gpu_kernel(const int n, const Dtype* data_col,
  ...
  CUDA_KERNEL_LOOP(index, n) {
    ...
    if (w_im % kernel_stride_w != 0 || h_im % kernel_stride_h != 0 ) {
        data_im[index] = val;
    } else {
      const int c_im = index / (width * height);
      const int kernel_h_eff =
          kernel_h + (kernel_h - 1) * (kernel_stride_h - 1);
      const int kernel_w_eff =
          kernel_w + (kernel_w - 1) * (kernel_stride_w - 1);
      // compute the start and end of the output
      const int w_col_start =
          (w_im < kernel_w_eff) ?
              w_im % kernel_stride_w : (w_im - kernel_w_eff) / stride_w + 1;
      const int w_col_end =
          min(w_im / stride_w + 1, width_col);
      const int h_col_start =
          (h_im < kernel_h_eff) ?
              h_im % kernel_stride_h : (h_im - kernel_h_eff) / stride_h + 1;
      const int h_col_end =
          min(h_im / stride_h + 1, height_col);
      for (int h_col = h_col_start; h_col < h_col_end; ++h_col) {
        for (int w_col = w_col_start; w_col < w_col_end; ++w_col) {
          int c_col = c_im * kernel_h * kernel_w
                + (h_im - h_col * stride_h) * kernel_w / kernel_stride_h
                + (w_im - w_col * stride_w) / kernel_stride_w;
...
void col2im_gpu(const Dtype* data_col, const int channels,
  ...
  col2im_gpu_kernel<Dtype><<<CAFFE_GET_BLOCKS(num_kernels),
      ...
      pad_h, pad_w, stride_h, stride_w, kernel_stride_h, kernel_stride_w,
...
__global__ void col2im_nd_gpu_kernel(const int n, const Dtype* data_col,
  ...
  CUDA_KERNEL_LOOP(index, n) {
    ...
    for (int i = 0; i < num_axes; ++i) {
      const int kernel_eff =
          kernel_shape[i] + (kernel_shape[i] - 1) * (kernel_stride[i] - 1);
      d_col_start[i] = d_col_iter[i] =
          (d_im[i] < kernel_eff) ?
              d_im[i] % kernel_stride[i] :
              (d_im[i] - kernel_eff) / stride[i] + 1;
      d_col_end[i] = min(d_im[i] / stride[i] + 1, col_shape[i + 1]);
      if (d_col_start[i] >= d_col_end[i] || d_im[i] % kernel_stride[i] != 0) {
        // Skip computation if the dimension is 0 at any spatial axis --
        // Also skip if kernel stride does not fall into the pixel index
        // final val will be 0.
        data_im[index] = 0;
        done = true;
    ...
    do {
      ...
      for (int i = num_axes - 1; i >= 0; --i) {
        final_offset += (d_im[i] - d_col_iter[i] * stride[i])
                        * kernel_shape_prod / kernel_stride[i];
