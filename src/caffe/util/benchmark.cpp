...
bool Timer::log(std::string file, record result) {
  ...
  if (Caffe::mode() == Caffe::CPU) {
    ...
    if (OpenCLManager::CurrentPlatform()->getNumDevices(CL_DEVICE_TYPE_CPU)
        > 0) {
      if (OpenCLManager::CurrentPlatform()->getDevice(CL_DEVICE_TYPE_CPU, 0)
        != NULL) {
        result.device =
            OpenCLManager::CurrentPlatform()->getDevice(CL_DEVICE_TYPE_CPU, 0)->name();   // NOLINT(*)
      }
    }
    result.sdk = "C++";
  }
  if (Caffe::mode() == Caffe::GPU) {
    result.device =
        OpenCLManager::CurrentPlatform()->getDevice(CL_DEVICE_TYPE_GPU, 0)->name();       // NOLINT(*)
