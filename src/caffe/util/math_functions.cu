...
__global__ void popc_kernel(const int_tp n, const float* a, const float* b,
                            uint8_t* y) {
  CUDA_KERNEL_LOOP(index, n) {
    y[index] = __popc(
...
__global__ void popcll_kernel(const int_tp n, const double* a, const double* b,
                              uint8_t* y) {
  CUDA_KERNEL_LOOP(index, n) {
    y[index] = __popcll(
...
template<>
uint32_t caffe_gpu_hamming_distance<float>(const int_tp n, const float* x,
                                           const float* y) {
  ...
  // NOLINT_NEXT_LINE(whitespace/operators)
  popc_kernel CUDA_KERNEL(CAFFE_GET_BLOCKS(n), CAFFE_CUDA_NUM_THREADS)(
...
template<>
uint32_t caffe_gpu_hamming_distance<double>(const int_tp n, const double* x,
                                            const double* y) {
  // TODO: Fix caffe_gpu_hamming_distance (see failing unit test
  // TestHammingDistanceGPU in test_math_functions.cpp).
  NOT_IMPLEMENTED;
  thrust::device_vector<uint8_t> popcounts(n);
  // NOLINT_NEXT_LINE(whitespace/operators)
  popcll_kernel CUDA_KERNEL(CAFFE_GET_BLOCKS(n), CAFFE_CUDA_NUM_THREADS)(
      n, x, y, thrust::raw_pointer_cast(popcounts.data()));
