...
template <>
void* fft_malloc<float>(int n) {
  //return (malloc(n));
  return((void*)fftwf_malloc(n));
}
template <>
void* fft_malloc<double>(int n) {
  // return (malloc(n));
  return((void*)fftw_malloc(n));
}

template <>
void fft_free<float>(void * p) {
 // free(p);
 fftwf_free(p);
}
template <>
void fft_free<double>(void * p) {
  // free(p);
  fftw_free(p);
}
template <>
...
template <>
void* fft_plan_dft_c2r_2d<float>(int n0, int n1, std::complex<float> *in, float *out, unsigned flags){
  fftwf_plan_dft_c2r_2d(n0,n1,(fftwf_complex *) in, out, flags);
}
template <>
void* fft_plan_dft_c2r_2d<double>(int n0, int n1, std::complex<double> *in, double *out, unsigned flags){
  fftw_plan_dft_c2r_2d(n0,n1,(fftw_complex *) in, out, flags);
}

template <>
void fft_destroy_plan<float>(void* plan){
	fftwf_destroy_plan((fftwf_plan)plan);
}
template <>
void fft_destroy_plan<double>(void* plan){
	fftw_destroy_plan((fftw_plan)plan);
}

template <>
void fft_execute_dft_r2c<float>(void* plan, float *in, std::complex<float> *out){
	fftwf_execute_dft_r2c((const fftwf_plan) plan, in, (fftwf_complex *) out);
}
template <>
void fft_execute_dft_r2c<double>(void* plan, double *in, std::complex<double> *out){
	fftw_execute_dft_r2c((const fftw_plan) plan, in, (fftw_complex *) out);
}
template <>
void fft_execute_dft_c2r<float>(void* plan, std::complex<float> *in, float *out){
	fftwf_execute_dft_c2r((const fftwf_plan) plan, (fftwf_complex *) in, out);
}
template <>
void fft_execute_dft_c2r<double>(void* plan, std::complex<double> *in, double *out){
