...
__global__ void caffe_gpu_interp2_kernel(const int n, const float rheight, const float rwidth,
    ...
    const Dtype *data1, const int x1, const int y1, const int height1, const int width1, const int Height1, const int Width1,
                                         Dtype *data2, const int x2, const int y2, const int height2, const int width2, const int Height2, const int Width2, const bool nn) {
  ...
  if (index < n) {
    ...
    if (packed) {
      ...
      for (int c = 0; c < channels; ++c) {
        if (nn)
          {
            if (h0lambda > h1lambda)
              if (w0lambda > w1lambda)
                pos2[0]= pos1[0];
              else
                pos2[0]= pos1[channels * w1p];
            else
              if (w0lambda > w1lambda)
                pos2[0]= pos1[channels * h1p * Width1];
              else
                pos2[0]= pos1[channels * (h1p * Width1 + w1p)];
          }
        else
          {
            pos2[0] =
        ...
        if (nn)
          {
            if (h0lambda > h1lambda)
              ...
              if (w0lambda > w1lambda)
                pos2[0] = pos1[h1p * Width1];
              else
                pos2[0] = pos1[h1p * Width1 + w1p];
          }
        else
          {
            pos2[0] =
...
void caffe_gpu_interp2(const int channels,
  ...
  caffe_gpu_interp2_kernel<Dtype,packed><<<CAFFE_GET_BLOCKS(num_kernels), CAFFE_CUDA_NUM_THREADS>>>
    (num_kernels, rheight, rwidth, channels,
     ...
     data2, x2, y2, height2, width2, Height2, Width2, nn);
...
__global__ void caffe_gpu_interp2_kernel_backward(const int n, const float rheight, const float rwidth,
    ...
    Dtype *data1, const int x1, const int y1, const int height1, const int width1, const int Height1, const int Width1,
                                                  const Dtype *data2, const int x2, const int y2, const int height2, const int width2, const int Height2, const int Width2, const bool nn) {
  int index = threadIdx.x + blockIdx.x * blockDim.x;
  if (index < n) {
    const int w2 = index % width2; // 0:width2-1
    const int h2 = index / width2; // 0:height2-1
        ...
        if (nn)
          {
            if (h0lambda > h1lambda)
              if (w0lambda > w1lambda)
                atomicAdd(&pos1[0], pos2[0]);
              else
                atomicAdd(&pos1[channels * w1p],  pos2[0]);
            else
              if (w0lambda > w1lambda)
                atomicAdd(&pos1[channels * h1p * Width1],  pos2[0]);
              else
                atomicAdd(&pos1[channels * (h1p * Width1 + w1p)],  pos2[0]);
          }
        else
          {
        ...
        if (nn)
          {
            if (h0lambda > h1lambda)
              if (w0lambda > w1lambda)
                ...
                atomicAdd(&pos1[w1p], pos2[0]);
            else
              if (w0lambda > w1lambda)
                atomicAdd(&pos1[h1p * Width1], pos2[0]);
              else
                atomicAdd(&pos1[h1p * Width1 + w1p], pos2[0]);
          }
        else
          {
            atomicAdd(&pos1[0], h0lambda * w0lambda * pos2[0]);
...
// Explicit instances
 template void caffe_gpu_interp2<float,false>(const int, const float *, const int, const int, const int, const int, const int, const int, float *, const int, const int, const int, const int, const int, const int, const bool nn);
 template void caffe_gpu_interp2<float,true>(const int, const float *, const int, const int, const int, const int, const int, const int, float *, const int, const int, const int, const int, const int, const int, const bool nn);
 template void caffe_gpu_interp2<double,false>(const int, const double *, const int, const int, const int, const int, const int, const int, double *, const int, const int, const int, const int, const int, const int, const bool nn);
 template void caffe_gpu_interp2<double,true>(const int, const double *, const int, const int, const int, const int, const int, const int, double *, const int, const int, const int, const int, const int, const int, const bool nn);

 template void caffe_gpu_interp2_backward<float,false>(const int, float *, const int, const int, const int, const int, const int, const int, const float *, const int, const int, const int, const int, const int, const int, const bool nn);
 template void caffe_gpu_interp2_backward<double,false>(const int, double *, const int, const int, const int, const int, const int, const int, const double *, const int, const int, const int, const int, const int, const int, const bool nn);
