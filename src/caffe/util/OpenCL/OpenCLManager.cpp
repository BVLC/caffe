...
OpenCLManager::OpenCLManager() : initialized_(
        false) {
}
...
bool OpenCLManager::Init() {
  ...
  if (!pf->createContext()) {
    LOG(FATAL)<< "failed to create OpenCL context for platform "
    << pf->name();
    return false;
  }
  ...
  cl_files.push_back(
      "src/caffe/util/OpenCL/gemm.cl");
  cl_files.push_back(
      "src/caffe/util/OpenCL/math_functions.cl");
  cl_files.push_back(
      "src/caffe/util/OpenCL/im2col.cl");
  cl_files.push_back(
      "src/caffe/layers/OpenCL/pooling_layer.cl");
  cl_files.push_back(
      "src/caffe/layers/OpenCL/relu_layer.cl");
  cl_files.push_back(
      "src/caffe/layers/OpenCL/prelu_layer.cl");
  cl_files.push_back(
      "src/caffe/layers/OpenCL/sigmoid_layer.cl");
  cl_files.push_back(
      "src/caffe/layers/OpenCL/tanh_layer.cl");
  cl_files.push_back(
      "src/caffe/layers/OpenCL/dropout_layer.cl");
  cl_files.push_back(
      "src/caffe/layers/OpenCL/bnll_layer.cl");
  cl_files.push_back(
      "src/caffe/layers/OpenCL/contrastive_loss_layer.cl");
  cl_files.push_back(
      "src/caffe/layers/OpenCL/eltwise_layer.cl");
  cl_files.push_back(
      "src/caffe/layers/OpenCL/lrn_layer.cl");
  cl_files.push_back(
      "src/caffe/layers/OpenCL/softmax_layer.cl");
  cl_files.push_back(
      "src/caffe/layers/OpenCL/softmax_loss_layer.cl");
  cl_files.push_back(
      "src/caffe/layers/OpenCL/threshold_layer.cl");
  cl_files.push_back(
      "src/caffe/layers/OpenCL/mvn_layer.cl");
  ...
  for (it = cl_files.begin(); it != cl_files.end(); it++) {
    if (!pf->compile(
        *it)) {
      LOG(FATAL)<< "failed to create to create OpenCL program for platform "
      << pf->name();
      return false;
    }
  }
...
void OpenCLManager::Print() {
  std::cout << "-- OpenCL Manager Information -------------------------------"
      << std::endl;
...
std::tr1::shared_ptr<OpenCLPlatform> OpenCLManager::getPlatform(
    unsigned int idx) {
