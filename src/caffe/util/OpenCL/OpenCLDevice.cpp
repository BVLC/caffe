...
namespace caffe {
  ...
  OpenCLDevice::~OpenCLDevice() {
    for (int i = 0; i < OPENCL_NUM_INPUT_QUEUES; i++) {
      if (inputQueues[i] != NULL) {
        ...
        LOG(INFO) << "release OpenCL input queue["
                  << i << "] on device " << name();
      }
    }
    ...
    for ( int i = 0; i < OPENCL_NUM_COMMAND_QUEUES; i++ ) {
      if (commandQueues[i] != NULL) {
        ...
        LOG(INFO) << "release OpenCL command queue["
                  << i << "] on device " << name();
      }
    }
    ...
    for ( int i = 0; i < OPENCL_NUM_OUTPUT_QUEUES; i++ ) {
      if (outputQueues[i] != NULL) {
        ...
        LOG(INFO) << "release OpenCL output queue["
                  << i << "] on device " << name();
      }
    }
  ...
  bool OpenCLDevice::query() {
    ...
    if (!CL_CHECK(
        clGetDeviceInfo(this->deviceID, CL_DEVICE_TYPE, sizeof(cl_device_type),
            &(this->deviceType), &size))) {
      return false;
    }
    if (!CL_CHECK(
        clGetDeviceInfo(this->deviceID, CL_DEVICE_MAX_COMPUTE_UNITS,
            sizeof(cl_uint), &(this->deviceMaxComputeUnits), &size))) {
      return false;
    }
    if (!CL_CHECK(
        clGetDeviceInfo(this->deviceID, CL_DEVICE_MAX_WORK_ITEM_DIMENSIONS,
            sizeof(cl_uint), &(this->deviceMaxWorkItemDims), &size))) {
      return false;
    }
    if (!CL_CHECK(
        clGetDeviceInfo(this->deviceID, CL_DEVICE_MAX_WORK_ITEM_SIZES,
            sizeof(size_t) * 3, &(this->deviceWorkItemSizes[0]), &size))) {
      return false;
    }
    if (!CL_CHECK(
        clGetDeviceInfo(this->deviceID, CL_DEVICE_MAX_WORK_GROUP_SIZE,
            sizeof(size_t), &(this->deviceMaxWorkGroupSize), &size))) {
      return false;
    }
    if (!CL_CHECK(
        clGetDeviceInfo(this->deviceID, CL_DEVICE_MAX_CLOCK_FREQUENCY,
            sizeof(cl_uint), &(this->deviceMaxClockFreqMHz), &size))) {
      return false;
    }
    if (!CL_CHECK(
        clGetDeviceInfo(this->deviceID, CL_DEVICE_MAX_MEM_ALLOC_SIZE,
            sizeof(cl_ulong), &(this->deviceMaxMemAllocSize), &size))) {
      return false;
    }
    if (!CL_CHECK(
        clGetDeviceInfo(this->deviceID, CL_DEVICE_MAX_PARAMETER_SIZE,
            sizeof(size_t), &(this->deviceMaxArgListSize), &size))) {
      return false;
    }
    if (!CL_CHECK(
        clGetDeviceInfo(this->deviceID, CL_DEVICE_MEM_BASE_ADDR_ALIGN,
            sizeof(cl_uint), &(this->deviceMemAddrAlign), &size))) {
      return false;
    }
    if (!CL_CHECK(
        clGetDeviceInfo(this->deviceID, CL_DEVICE_MIN_DATA_TYPE_ALIGN_SIZE,
            sizeof(cl_uint), &(this->deviceMinMemAddrAlign), &size))) {
      return false;
    }
    if (!CL_CHECK(
        clGetDeviceInfo(this->deviceID, CL_DEVICE_GLOBAL_MEM_CACHE_TYPE,
            sizeof(cl_device_mem_cache_type),
            &(this->deviceGlobalMemCacheType), &size))) {
      return false;
    }
    if (!CL_CHECK(
        clGetDeviceInfo(this->deviceID, CL_DEVICE_GLOBAL_MEM_CACHE_SIZE,
            sizeof(cl_ulong), &(this->deviceGlobalMemCacheSize), &size))) {
      return false;
    }
    if (!CL_CHECK(
        clGetDeviceInfo(this->deviceID, CL_DEVICE_GLOBAL_MEM_CACHELINE_SIZE,
            sizeof(cl_uint), &(this->deviceGlobalMemCacheLineSize), &size))) {
      return false;
    }
    if (!CL_CHECK(
        clGetDeviceInfo(this->deviceID, CL_DEVICE_GLOBAL_MEM_SIZE,
            sizeof(cl_ulong), &(this->deviceGlobalMemSize), &size))) {
      return false;
    }
    if (!CL_CHECK(
        clGetDeviceInfo(this->deviceID, CL_DEVICE_LOCAL_MEM_TYPE,
            sizeof(cl_device_local_mem_type),
            &(this->deviceLocalMemType), &size))) {
      return false;
    }
    if (!CL_CHECK(
        clGetDeviceInfo(this->deviceID, CL_DEVICE_LOCAL_MEM_SIZE,
            sizeof(cl_ulong), &(this->deviceLocalMemSize), &size))) {
      return false;
    }
    if (!CL_CHECK(
        clGetDeviceInfo(this->deviceID, CL_DEVICE_HOST_UNIFIED_MEMORY,
            sizeof(cl_bool), &(this->deviceHostUnifiedMem), &size))) {
      return false;
    }
    if (!CL_CHECK(
        clGetDeviceInfo(this->deviceID, CL_DEVICE_HOST_UNIFIED_MEMORY,
    ...
    if (!CL_CHECK(
        clGetDeviceInfo(this->deviceID, CL_DEVICE_MEM_BASE_ADDR_ALIGN,
            sizeof(cl_uint), &(this->deviceMemBaseAddrAlign), &size))) {
      return false;
    }
    size = 1024;
    char* name = reinterpret_cast<char*>(malloc(size * sizeof(char)));
    if (!CL_CHECK(
        clGetDeviceInfo(this->deviceID, CL_DEVICE_NAME, size, name, NULL))) {
      return false;
    }
  ...
  void OpenCLDevice::print() {
    std::cout
        ...
        << std::endl;
    std::cout << "    deviceType                    = " << this->deviceTypeStr
        << std::endl;
    std::cout << "    deviceName                    = " << this->deviceName
        << std::endl;
    std::cout << "    deviceMaxComputeUnits         = "
        << this->deviceMaxComputeUnits << std::endl;
    std::cout << "    deviceMaxWorkItemDims         = "
        << this->deviceMaxWorkItemDims << std::endl;
    std::cout << "    deviceMaxWorkGroupSize        = "
        << this->deviceMaxWorkGroupSize << std::endl;
    std::cout << "    deviceMaxClockFreqMHz         = "
        << this->deviceMaxClockFreqMHz << std::endl;
    std::cout << "    deviceMaxMemAllocSize         = "
        << this->deviceMaxMemAllocSize << std::endl;
    std::cout << "    deviceMaxArgListSize          = "
        << this->deviceMaxArgListSize << std::endl;
    std::cout << "    deviceMemAddrAlign            = "
        << this->deviceMemAddrAlign << std::endl;
    std::cout << "    deviceMinMemAddrAlign         = "
        << this->deviceMinMemAddrAlign << std::endl;
    std::cout << "    deviceGlobalMemCacheType      = "
        << this->deviceGlobalMemCacheType << std::endl;
    std::cout << "    deviceGlobalMemCacheSize      = "
        << this->deviceGlobalMemCacheSize << std::endl;
    std::cout << "    deviceGlobalMemCacheLineSize  = "
        << this->deviceGlobalMemCacheLineSize << std::endl;
    std::cout << "    deviceGlobalMemSize           = "
        << this->deviceGlobalMemSize << std::endl;
    std::cout << "    deviceLocalMemType            = "
        << this->deviceLocalMemType << std::endl;
    std::cout << "    deviceLocalMemSize            = "
        << this->deviceLocalMemSize << std::endl;
    std::cout << "    deviceHostUnifiedMem          = "
        << this->deviceHostUnifiedMem << std::endl;
    std::cout << "    deviceMemBaseAddrAlign        = "
        << this->deviceMemBaseAddrAlign << std::endl;
  }
  ...
  bool OpenCLDevice::compile(std::string cl_source) {
    ...
    if ( access(cl_source.c_str(), F_OK) == -1 ) {
      LOG(ERROR) << "kernel source file = '"
                 << cl_source.c_str() << "' doesn't exist";
      return false;
    }

    if ( access(cl_source.c_str(), R_OK) == -1 ) {
      LOG(ERROR) << "kernel source file = '"
                 << cl_source.c_str() << "' isn't readable";
      return false;
    }
    ...
    if ( !parser.convert(cl_source, cl_standard) ) {
      LOG(ERROR) << "failed to convert kernel source file = '"
                 << cl_source.c_str() << "' to standard OpenCL";
      return false;
    }

    std::vector<std::string> kernel_names;
    if ( !parser.getKernelNames(cl_standard, kernel_names) ) {
      LOG(ERROR) << "failed to parse kernel names from file '"
                 << cl_standard.c_str() << "'";
      return false;
    }
    ...
    std::string str(
        std::istreambuf_iterator<char>(
            std::ifstream(cl_standard.c_str()).rdbuf()),
            std::istreambuf_iterator<char>());
    if ( str.size() <= 0 ) {
    ...
    cl_program program =
        clCreateProgramWithSource(context_(), 1, &list, sourceSize, &err);
    if ( err != CL_SUCCESS ) {
      LOG(ERROR) << "failed to create program from file = '"
                 << cl_standard.c_str() << "'";
      return false;
    }
    DLOG(INFO) << "create program with source from file = '"
               << cl_standard.c_str() << "' for device " << this->name();

    // -x clc++ -O5 -cl-std=CL2.0
    std::string clIncludes = "-cl-unsafe-math-optimizations "
        "-cl-finite-math-only "
        ...
        "-cl-mad-enable "
        "-cl-no-signed-zeros "
        "-I ../../CL/include/ -I include/caffe/util/OpenCL/";

    err = clBuildProgram(program, 1, &deviceID, clIncludes.c_str(), NULL, NULL);
    if ( err != CL_SUCCESS ) {
      LOG(ERROR) << "failed to build OpenCL program from file '"
                 << cl_standard.c_str() << "' : error = " << err;

      char* logBuffer = reinterpret_cast<char*>(malloc(1024 * 1024));
      size_t tempSize;

      err = clGetProgramBuildInfo(program,
          deviceID, CL_PROGRAM_BUILD_LOG, 1000000, logBuffer, &tempSize);
      if ( err != CL_SUCCESS ) {
      ...
      if ( tempLogFile == NULL ) {
        LOG(ERROR) << "failed to open build log file '"
                   << os.str().c_str() << "'";
      } else {
        fwrite(logBuffer, 1, tempSize, tempLogFile);
        fclose(tempLogFile);
        DLOG(INFO) << "OpenCL build log written to file '"
                   << os.str().c_str() << "'";
      }

      return false;
    }
    DLOG(INFO) << "create program from file = '" << cl_standard.c_str()
               << "' for device " << this->name();
    ...
    for ( it = kernel_names.begin(); it != kernel_names.end(); it++ ) {
      ...
      if ( err != CL_SUCCESS ) {
        LOG(ERROR) << "failed to create kernel '" << (*it).c_str()
                   << "' from file '" << cl_standard.c_str()
                   << "' : error = " << caffe::OpenCL::what(err);
        return false;
      }
      ...
      kernel_map_[(*it)] = kern;
      DLOG(INFO) << "create kernel '" << (*it).c_str() << "' from file '"
                 << cl_standard.c_str() << "' for device " << this->name()
                 << " @ " << kernel_map_[(*it)];
    }
  ...
  bool OpenCLDevice::createQueue() {
    ...
    cl_int err;
    for ( int i = 0; i < OPENCL_NUM_INPUT_QUEUES; i++ ) {
      inputQueues[i] = clCreateCommandQueue(context_(), deviceID, props, &err);
      if ( err != CL_SUCCESS ) {
        LOG(ERROR) << "failed to create OpenCL input queue["
                   << i << "] for device " << this->name();
        return false;
      }
      LOG(INFO) << "create OpenCL input queue[" << i << "] for device "
                << this->name() << " @ queues = " << inputQueues[i]
                << " and deviceID = " << deviceID;
    }

    for ( int i = 0; i < OPENCL_NUM_COMMAND_QUEUES; i++ ) {
      commandQueues[i] =
        clCreateCommandQueue(context_(), deviceID, props, &err);
      if ( err != CL_SUCCESS ) {
        LOG(ERROR) << "failed to create OpenCL command queue["
                   << i << "] for device " << this->name();
        return false;
      }
      LOG(INFO) << "create OpenCL command queue[" << i << "] for device "
                << this->name() << " @ queues = " << commandQueues[i]
                << " and deviceID = " << deviceID;
    }

    for ( int i = 0; i < OPENCL_NUM_OUTPUT_QUEUES; i++ ) {
      outputQueues[i] = clCreateCommandQueue(context_(), deviceID, props, &err);
      if ( err != CL_SUCCESS ) {
        LOG(ERROR) << "failed to create OpenCL output queue["
                   << i << "] for device " << this->name();
        return false;
      }
      LOG(INFO) << "create OpenCL output queue[" << i << "] for device "
                << this->name() << " @ queues = " << outputQueues[i]
                << " and deviceID = " << deviceID;
    }
  ...
  cl_command_queue* OpenCLDevice::getNextCommandQueue() {
    ...
    DLOG(INFO) << "really next OpenCL command queue = "
              << currentCommandQueueIdx;
  ...
  bool OpenCLDevice::add(OpenCLMemory& clMem) {  // NOLINT(*)
    ...
    if ( memory.find(clMem.getVirtualPointer()) != memory.end() ) {
      LOG(ERROR) << this->name() << " memory already exists : "
                 << clMem.getTag().c_str();
      return false;
    }
  ...
  bool OpenCLDevice::getBuffer(
      std::tr1::shared_ptr<OpenCLBuffer>* ptr,
      size_t size) {
    if (size < 1) {
    ...
    if ( buffer.find(size) == buffer.end() ) {
      DLOG(INFO) << this->name() << " no buffer of size = "
                 << size << " found, create new buffer.";
    ...
    for ( it = buffer[size].begin(); it != buffer[size].end(); it++ ) {
      if ( (*it)->isAvailable() == true ) {
        DLOG(INFO) << this->name() << " found available buffer of size = "
                   << size;
        (*it)->setUnavailable();
        *ptr = (*it);
        return true;
    ...
    DLOG(INFO) << this->name() << " found no available buffer of size = "
               << size << " found, create new buffer.";
    *ptr = std::tr1::shared_ptr<OpenCLBuffer>(new OpenCLBuffer(size));
    buffer[size].push_back(*ptr);
    this->add(**ptr);
  ...
  bool OpenCLDevice::setBufferAvailable(const void* ptr, size_t size) {
    ...
    if ( buffer.find(size) == buffer.end() ) {
      DLOG(ERROR) << this->name() << " no buffer of size = "
                  << size << " exists.";
      return false;
    }
    ...
    for ( it = buffer[size].begin(); it != buffer[size].end(); it++ ) {
      if ( (*it)->getVirtualPointer() == ptr ) {
        DLOG(INFO) << this->name() << " set buffer of size = "
                   << size << " with virtual address = "
                   << ptr << " to available";
        (*it)->setAvailable();
        return true;
      }
    ...
    DLOG(ERROR) << this->name() << " found no buffer of size = "
                << size << " with virtual address = " << ptr;
    return false;
  }

  bool OpenCLDevice::rmMemoryPtr(const void* ptr) {
    if (!isValidPtr(ptr)) {
      LOG(ERROR) << this->name() << " not a valid pointer @ "
                << ptr << " on this device.";
      return false;
    }

    if ( memory.find(ptr) == memory.end() ) {
      LOG(ERROR) << this->name() << " not a valid key @ "
                 << ptr << " on this device.";
      return false;
    }
    try {
    ...
    if ( memory.find(ptr) != memory.end() ) {
      LOG(ERROR) << this->name() << " ptr @ "
                 << ptr << " still on this device.";
      return false;
    }
