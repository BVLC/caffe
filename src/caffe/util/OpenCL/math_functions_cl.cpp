...
namespace caffe {
  ...
  void caffe_gpu_memcpy(const size_t N, const void* X, void* Y, int type) {
    if (X != Y) {
      BOOL_CHECK(caffe::OpenCL::clMemcpy(Y, X, N, type));  // NOLINT(*)
    }
  }

  void caffe_gpu_memcpy(const size_t N, const void* X, void* Y) {
    if (X != Y) {
      BOOL_CHECK(caffe::OpenCL::clMemcpy(Y, X, N, caffe::OpenCL::COPY_DEFAULT)); // NOLINT(*)
    }
  }
  ...
  template void caffe_gpu_memset<char>(const size_t Bytes, const char alpha, void* X);        // NOLINT(*)
  template void caffe_gpu_memset<int>(const size_t Bytes, const int alpha, void* X);          // NOLINT(*)
  template void caffe_gpu_memset<float>(const size_t Bytes, const float alpha, void* X);      // NOLINT(*)
  template void caffe_gpu_memset<double>(const size_t Bytes, const double alpha, void* X);    // NOLINT(*)
  ...
  template void caffe_gpu_sgnbit<double>(const int n, const double* x, double* y);            // NOLINT(*)
  ...
  template void caffe_gpu_div<float>(const int n, const float* x, const float* y, float* z);      // NOLINT(*)
  template void caffe_gpu_div<double>(const int n, const double* x, const double* y, double* z);  // NOLINT(*)

  template<typename T>
  void caffe_gpu_mul(const int n, const T* x, const T* y, T* z) {
    BOOL_CHECK(caffe::OpenCL::clmul<T>(n, x, y, z));
  }
  template void caffe_gpu_mul<float>(const int n, const float* x, const float* y, float* z);      // NOLINT(*)
  template void caffe_gpu_mul<double>(const int n, const double* x, const double* y, double* z);  // NOLINT(*)

  template<typename T>
  void caffe_gpu_scale(const int n, const float alpha, const T* x, T* y) {
    BOOL_CHECK(caffe::OpenCL::clBLASscal<T>(n, alpha, x, y));
  }
  template void caffe_gpu_scale<float>(const int n, const float alpha, const float* x, float* y);     // NOLINT(*)
  template void caffe_gpu_scale<double>(const int n, const float alpha, const double* x, double* y);  // NOLINT(*)
  ...
  template void caffe_gpu_set<int>(const int N, const int alpha, int* Y);             // NOLINT(*)
  template void caffe_gpu_set<float>(const int N, const float alpha, float* Y);       // NOLINT(*)
  template void caffe_gpu_set<double>(const int N, const double alpha, double* Y);    // NOLINT(*)

  template<typename T>
  void caffe_gpu_dot(const int n, const T* x, const T* y, T* out) {
    BOOL_CHECK(caffe::OpenCL::clBLASdot<T>(n, x, 1, y, 1, out));
  }
  template void caffe_gpu_dot<float>(const int n, const float* x, const float* y, float* out);        // NOLINT(*)
  template void caffe_gpu_dot<double>(const int n, const double* x, const double* y, double* out);    // NOLINT(*)

  template<typename T>
  void caffe_gpu_sub(const int n, const T* a, const T* b, T* y) {
    BOOL_CHECK(caffe::OpenCL::clsub<T>(n, a, b, y));
  }
  template void caffe_gpu_sub<float>(const int n, const float* a, const float* b, float* y);          // NOLINT(*)
  template void caffe_gpu_sub<double>(const int n, const double* a, const double* b, double* y);      // NOLINT(*)

  template<typename T>
  void caffe_gpu_add(const int n, const T* a, const T* b, T* y) {
    BOOL_CHECK(caffe::OpenCL::cladd<T>(n, a, b, y));
  }
  template void caffe_gpu_add<float>(const int n, const float* a, const float* b, float* y);          // NOLINT(*)
  template void caffe_gpu_add<double>(const int n, const double* a, const double* b, double* y);      // NOLINT(*)

  template<typename T>
  void caffe_gpu_add_scalar(const int N, const T alpha, T* Y) {
    BOOL_CHECK(caffe::OpenCL::cladd_scalar(N, alpha, Y));
  }
  template void caffe_gpu_add_scalar<float>(const int N, const float alpha, float* Y);                // NOLINT(*)
  template void caffe_gpu_add_scalar<double>(const int N, const double alpha, double* Y);             // NOLINT(*)

  template<typename T>
  void caffe_gpu_powx(const int n, const T* a, const T alpha, T* y) {
    BOOL_CHECK(caffe::OpenCL::clpowx<T>(n, a, alpha, y));
  }
  template void caffe_gpu_powx<float>(const int n, const float* a, const float alpha, float* y);      // NOLINT(*)
  template void caffe_gpu_powx<double>(const int n, const double* a, const double alpha, double* y);  // NOLINT(*)
...
#ifdef USE_CLGEMM
      BOOL_CHECK(caffe::OpenCL::clgemv<T>(clTransA, m, n, alpha, A, x, beta, y));     // NOLINT(*)
#else
    BOOL_CHECK(caffe::OpenCL::clBLASgemv<T>(clTransA, m, n, alpha, A, x, beta, y));   // NOLINT(*)
#endif
  }
...
#ifdef USE_CLGEMM
      BOOL_CHECK(caffe::OpenCL::clgemv<T>(
          clTransA, m, n, alpha, A, step_A, x, step_x, beta, y, step_y));
#else
    BOOL_CHECK(
    caffe::OpenCL::clBLASgemv<T>(
        clTransA, m, n, alpha, A, step_A, x, step_x, beta, y, step_y));
#endif
  }
...
#ifdef USE_CLGEMM
      BOOL_CHECK(caffe::OpenCL::clgemm<T>(
          clTransA, clTransB, M, N, K, alpha, A, B, beta, C, event));
#else
    BOOL_CHECK(caffe::OpenCL::clBLASgemm<T>(
        clTransA, clTransB, M, N, K, alpha, A, B, beta, C, event));
#endif
  }
  ...
  void caffe_gpu_gemm(
      ...
      const T beta,
      T* C) {
    return caffe_gpu_gemm<T>(TransA, TransB, M, N, K, alpha, A, B, beta, C, NULL);  // NOLINT(*)
  }
  template void caffe_gpu_gemm<float>(
      const CBLAS_TRANSPOSE TransA,
      const CBLAS_TRANSPOSE TransB,
...
#ifdef USE_CLGEMM
      BOOL_CHECK(caffe::OpenCL::clgemm<T>(
          clTransA, clTransB, M, N, K,
          alpha,
          A, idx_offset_A,
          B, idx_offset_B,
          beta,
          C, idx_offset_C, event));
#else
    BOOL_CHECK(caffe::OpenCL::clBLASgemm<T>(
        clTransA, clTransB, M, N, K,
        alpha,
...
#ifdef USE_CLGEMM
      BOOL_CHECK(caffe::OpenCL::clgemm<T>(clTransA, clTransB, M, N, K,
          alpha,
          A, idx_offset_A,
          B, idx_offset_B,
          beta,
          C, idx_offset_C, event));
#else
    BOOL_CHECK(caffe::OpenCL::clBLASgemm<T>(clTransA, clTransB, M, N, K,
        alpha,
        A, idx_offset_A, lda,
        B, idx_offset_B, ldb,
        beta,
        C, idx_offset_C, ldc, event));
#endif
  }
  template void caffe_gpu_gemm<float>(
      const CBLAS_TRANSPOSE TransA,
  ...
  void caffe_gpu_gemm(
      ...
      T* C,
      const size_t idx_offset_C) {
    return caffe_gpu_gemm<T>(TransA, TransB, M, N, K,
        alpha,
        A, idx_offset_A,
        B, idx_offset_B,
        beta,
  ...
  void caffe_gpu_group_gemm(
    ...
    BOOL_CHECK(caffe::OpenCL::cl_group_gemm<T>(clTransA, clTransB, M, N, K,
        GM, GN, GK, alpha, A, 0, B, 0, beta, C, 0, NULL));
  }
  ...
  template void caffe_gpu_axpy<float>(const int N, const float alpha, const float* X, float* Y);      // NOLINT(*)
  template void caffe_gpu_axpy<double>(const int N, const double alpha, const double* X, double* Y);  // NOLINT(*)

  template<typename T>
  void caffe_gpu_axpby(const int N, const T alpha, const T* X, const T beta, T* Y) {  // NOLINT(*)
    TIME("caffe_gpu_scal()", {
  ...
  template void caffe_gpu_scal<float>(const int N, const float alpha, float *X);      // NOLINT(*)
  template void caffe_gpu_scal<double>(const int N, const double alpha, double *X);   // NOLINT(*)
  ...
  template void caffe_gpu_rng_uniform<float>(const int n, const float a, const float b, float* r);  // NOLINT(*)
  ...
  template void caffe_gpu_rng_bernoulli<float, int>(const int n, const float p, int* r);    // NOLINT(*)
  template void caffe_gpu_rng_bernoulli<double, int>(const int n, const double p, int* r);  // NOLINT(*)
