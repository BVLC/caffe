...
bool OpenCLParser::getKernelNames(
    std::string fileName,
    std::vector<std::string>& names) {  // NOLINT(*)
  if (access(
      fileName.c_str(),
      F_OK) == -1) {
  ...
  boost::regex kernel_line("^__kernel[[:space:]]+void[[:space:]]+[[:word:]]+\\(.*", boost::regex::perl);  // NOLINT(*)
  boost::regex template_kernel_line("^template\\s+<class \\w+>\\s+__kernel[[:space:]]+void[[:space:]]+[[:word:]]+\\(.*", boost::regex::perl);  // NOLINT(*)
  boost::regex split("[\\s+(]");
  ...
  while (std::getline(file, str)) {
    ...
    if ( boost::regex_match(str, what,
                            template_kernel_line, boost::match_default) ) {
      for (int m = 0; m < what.size(); m++) {
        boost::sregex_token_iterator it(str.begin(), str.end(), split, -1);
        it++;
        it++;
...
bool OpenCLParser::isKernelLine(std::string line) {
  boost::regex re(
      "^__kernel[[:space:]]+void[[:space:]]+[[:word:]]+\\(.*",
      boost::regex::perl);
  return this->match(
      line,
      re);
...
bool OpenCLParser::isTemplateKernelLine(std::string line) {
  boost::regex re("^template\\s+<class \\w+>\\s+__kernel[[:space:]]+void[[:space:]]+[[:word:]]+\\(.*", boost::regex::perl);  // NOLINT(*)
  return this->match(
      line,
      re);
}
...
bool OpenCLParser::isAttributeLine(std::string line) {
  boost::regex re(
      "^template __attribute__\\(\\(mangled_name\\(.*",
      boost::regex::perl);
  return this->match(
      line,
      re);
...
std::string OpenCLParser::getTypedKernelName(std::string line) {
  boost::regex split_bgn(
      "template\\s+__attribute__\\(\\(mangled_name\\(");
  boost::regex split_end(
      "\\)");
  boost::sregex_token_iterator it;
...
bool OpenCLParser::convert(std::string fileNameIN, std::string fileNameOUT) {
  if (access(fileNameIN.c_str(), F_OK) == -1) {
    LOG(ERROR) << "kernel source file = '"
               << fileNameIN.c_str() << "' doesn't exist";
    return false;
  }

  if (access(fileNameIN.c_str(), R_OK) == -1) {
    LOG(ERROR) << "kernel source file = '"
               << fileNameIN.c_str() << "' isn't readable";
    return false;
  }
  ...
  if (access(fileNameOUT.c_str(), F_OK) == 0) {
    ...
    if ( statOUT.st_mtime > statIN.st_mtime ) {
      DLOG(INFO) << "kernel source file = '"
      << fileNameOUT.c_str() << "' up-to-date";
      return true;
    }
  }
  ...
  stdOpenCL += "// This file was auto-generated from file '"
            + fileNameIN
            + "' to conform to standard OpenCL\n";
  ...
  while (std::getline(file, line)) {
    if ( isAttributeLine(line) ) {
      ...
      kernel_line_typed = "__kernel void " + kernel_name_typed + getTypedKernelLine(line) + " {";  // NOLINT(*)
      ...
      kernel_modified = boost::regex_replace(kernel_modified, re, " "+type_replace+" ");  // NOLINT(*)
    ...
    if ( isTemplateKernelLine(line) ) {
      ...
      DLOG(INFO) << "found template kernel '"
      << kernel_name << "' with type '" << kernel_type << "'";

      if ( recording == false ) {
        recording = true;
      } else {
        LOG(ERROR) << "error parsing kernel source file = '"
        << fileNameIN.c_str() << "'";
        return false;
      }
      continue;
  ...
  out << stdOpenCL;
  out.close();
  DLOG(INFO) << "convert AMD OpenCL '" << fileNameIN.c_str()
  << "' to standard OpenCL '" << fileNameOUT.c_str() << "'";
  return true;
}
