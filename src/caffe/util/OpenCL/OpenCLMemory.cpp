...
uint64_t caffe::OpenCLMemory::count           = 0;
uint64_t caffe::OpenCLMemory::numCallsMalloc  = 0;
uint64_t caffe::OpenCLMemory::numCallsFree    = 0;

void* caffe::OpenCLMemory::ptr_offset = reinterpret_cast<void*>(1UL
    << CL_PTR_BIT);
...
bool OpenCLMemory::overlaps(OpenCLMemory& clMem) {  // NOLINT(*)
  if (this->ptr_device_mem == NULL) {
    return false;
  }
...
bool OpenCLMemory::includes(OpenCLMemory& clMem) {  // NOLINT(*)
  if (this->ptr_device_mem == NULL) {
    return false;
  }
  if (clMem.ptr_device_mem == NULL) {
  ...
  if (this->ptr_virtual_bgn <= clMem.ptr_virtual_bgn
      && this->ptr_virtual_end >= clMem.ptr_virtual_bgn) {
    return true;
  }
  return false;
}
...
cl_event OpenCLMemory::getEvent() {
  if (this->memoryEvent != NULL) {
    DLOG(INFO)<< "get event for " << this->getTag() << " event = "
    << this->memoryEvent;
