...
template <typename Dtype>
void Net<Dtype>::show_blob_mean(const string& blob_name) {
 if ( has_blob(blob_name) ) {
   shared_ptr<Blob<Dtype> > blob_ptr = blob_by_name( blob_name );
   Dtype mean=0;
   for (int i=0; i<blob_ptr->count(); i++)
     mean += blob_ptr->cpu_data()[i];
   LOG(INFO) << "The mean of " << blob_name << " is: " << mean / (Dtype)blob_ptr->count();
 } else
   LOG(INFO) << "No blob named " << blob_name;
}
