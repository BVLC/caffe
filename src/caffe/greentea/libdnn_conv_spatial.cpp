...
LibDNNConvSpatial<Dtype>::LibDNNConvSpatial(LibDNNConvConfig config) {
  ...
  is_1x1_ = true;
  for (int_tp i = 0; i < spatial_dims; ++i) {
     is_1x1_ &= kernel_shape_[i] == 1 && stride_[i] == 1 && pad_[i] == 0;
     if (!is_1x1_) {
        break;
     }
  }
...
template<typename Dtype>
const LibDNNConvConfig LibDNNConvSpatial<Dtype>::get_config() {
  return config_;
}
...
std::string LibDNNConvSpatial<Dtype>::generate_fw_defs() {
  ...
  LibDNN<Dtype>::add_def(ss, "KERNEL_WIDTH", kernel_w_);
  LibDNN<Dtype>::add_def(ss, "KERNEL_HEIGHT" , kernel_h_);
  LibDNN<Dtype>::add_def(ss, "STRIDE_X", stride_w_);
  LibDNN<Dtype>::add_def(ss, "STRIDE_Y", stride_h_);
  LibDNN<Dtype>::add_def(ss, "DILATION_X", dilation_w_);
  LibDNN<Dtype>::add_def(ss, "DILATION_Y", dilation_h_);
  LibDNN<Dtype>::add_def(ss, "INPUT_PAD_W", pad_w_);
  LibDNN<Dtype>::add_def(ss, "INPUT_PAD_H", pad_h_);
...
std::string LibDNNConvSpatial<Dtype>::generate_fw_kernels(int_tp kernelType,
  ...
  if (kernelType == KERNEL_TYPE_INTEL_IDLF) {
    ...
    LibDNN<Dtype>::add_def(ss, "INPUT_DEPTH", channels_ / group_);
    LibDNN<Dtype>::add_def(ss, "TOTAL_INPUT_DEPTH_SIZE", channels_);
    LibDNN<Dtype>::add_def(ss, "TOTAL_OUTPUT_DEPTH", num_output_);
    LibDNN<Dtype>::add_def(ss, "INPUT_START_X", 0);
    LibDNN<Dtype>::add_def(ss, "INPUT_START_Y", 0);
    LibDNN<Dtype>::add_def(ss, "INPUT_START_Z", 0);
    LibDNN<Dtype>::add_def(ss, "NUM_FILTERS", M_);
    ...
    LibDNN<Dtype>::add_def(ss, "ALIGNED_NUM_FILTERS", ALIGN(M_, simd_size));
    ...
    LibDNN<Dtype>::add_def(ss, "INPUT_DEPTH", channels_);
    LibDNN<Dtype>::add_def(ss, "WIDTH1", M_);
    LibDNN<Dtype>::add_def(ss, "OUT_PADDING_LEFT", 0);
    LibDNN<Dtype>::add_def(ss, "OUT_PADDING_HEIGHT", 0);
    LibDNN<Dtype>::add_def(ss, "OUT_DEPTH", M_);
    LibDNN<Dtype>::add_def(ss, "KERNEL_WIDTH_DIV2", kernel_w_ / 2);
    LibDNN<Dtype>::add_def(ss, "KERNEL_SLICE_DIV2", (kernel_w_*kernel_h_)/2);
    LibDNN<Dtype>::add_def(ss, "TILE_N_LAST", M_ % 32);
...
#define TYPEDEF_FLOAT_N(ele_num) \
    ...
    LibDNN<Dtype>::add_def(ss, "TILE_K", kernel_w_);
...
#define GENERATE_DOT_PRODUCT(ele_num) \
    ...
    // defs
    LibDNN<Dtype>::add_def(ss, "CHANNELS", channels_ / group_);
    LibDNN<Dtype>::add_def(ss, "APPLY_BIAS", bias_term_);
    LibDNN<Dtype>::add_def(ss, "OUTPUT_Z", M_);
...
void LibDNNConvSpatial<Dtype>::Backward(bool prop_down_data,
                                        ...
                                        Dtype* bottom_diff,
                                        int_tp batch_size) {
  printf("Backward: Not implemented yet\n");
}
...
void LibDNNConvSpatial<Dtype>::swizzleWeights(
  ...
  bool phase_test = get_config().phase_test;
  if (tuned_ &&
      swizzled_weights_ != NULL &&
      phase_test == true)
    return;
  ...
  if (!interleave) {
    ...
    Dtype* cpu_weight = reinterpret_cast<Dtype*>(clEnqueueMapBuffer(
             ...
             0, sizeof(Dtype) * num_output_ * kernel_dim_ * group_,
             0, NULL, NULL, NULL));
    ...
    size_t interleaved_filter_size =
       M_ * kernel_w_ * kernel_h_ * channels_ * sizeof(Dtype);
