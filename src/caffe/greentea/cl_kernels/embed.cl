...
// atomic_add fddrom: http://suhorukov.blogspot.com/2011/12/opencl-11-atomic-operations-on-floating.html
#if (TYPE == TYPE_HALF)

// FIXME, has bug which may hang GPU.
inline void TEMPLATE(atomic_add,Dtype)(volatile __global Dtype *source, const Dtype operand) {
    ...
    do {
        // FIXME, need to consider buffer overflow.
        prevVal.floatVal[0] = *source;
        prevVal.floatVal[1] = *(source+1);
        newVal.floatVal[0] = prevVal.floatVal[0] + operand;
        newVal.floatVal[1] = prevVal.floatVal[1];
