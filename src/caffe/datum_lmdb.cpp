...
shared_ptr<DatumDB::Generator> DatumLMDB::GetGenerator() {
  CHECK_EQ(param_.mode(),DatumDBParameter_Mode_READ)
    << "Only DatumDB in Mode_READ can use GetGenerator";
  DatumLMDB* datumdb = new DatumLMDB(param_);
  datumdb->mdb_env_ = mdb_env_;
  datumdb->mdb_txn_ = mdb_txn_;
  datumdb->mdb_dbi_ = mdb_dbi_;
