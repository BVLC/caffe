...
void AdaGradSolver<Dtype>::GenerateProgram() {
  ...
  ss << this->device_program_->function("AdaGradUpdate", args);
  ss << this->device_program_->kernel_loop("uint_tp", "i", "n");
  ss << "Dtype gi = g[i];" << std::endl;
  ss << "Dtype hi = h[i] = h[i] + gi * gi;" << std::endl;
  ss << "g[i] = local_rate * gi / (sqrt(hi) + delta);" << std::endl;
  ss << "}" << std::endl;
  ss << "}" << std::endl;
...
void adagrad_update_gpu(Device* dev, DeviceProgram* dev_prog, uint_tp n,
                        ...
                        Dtype local_rate) {
  shared_ptr<DeviceKernel> kernel = dev_prog->GetKernel("AdaGradUpdate");
  kernel->add_arg(&n);
  kernel->add_arg(&g);
  kernel->add_arg(&h);
