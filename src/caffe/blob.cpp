...
#ifdef USE_GREENTEA
        greentea_gpu_dot(device_context_.id(), count_, (cl_mem) data, 0,
                         (cl_mem) data, 0, &sumsq);
#endif
      }
#else
...
#ifdef USE_GREENTEA
        greentea_gpu_dot(device_context_.id(), count_, (cl_mem) diff, 0,
                         (cl_mem) diff, 0, &sumsq);
#endif
      }
#else
...
#ifdef USE_GREENTEA
        greentea_gpu_scal(device_context_.id(), count_, scale_factor,
                          (cl_mem) data, 0);
#endif
      }
      return;
