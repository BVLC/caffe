...
MslSolver<Dtype>::MslSolver(shared_ptr<Solver<Dtype> > root_solver)
  : root_solver_(root_solver)
  , iter_size(root_solver->param().iter_size())
  , multi_node(MSL::GetNumNodes() > 1) {

  root_solver->set_forward_backward(
    boost::bind(&MslSolver<Dtype>::ForwardBackward, this));
}

template <typename Dtype>
Dtype MslSolver<Dtype>::ForwardBackwardImpl(bool first, bool last) {
...
template <typename Dtype>
Dtype MslSolver<Dtype>::ForwardBackward() {
  Dtype loss = 0;
...
template <typename Dtype>
void MslSolver<Dtype>::Solve() {
  root_solver_->Solve();
}

INSTANTIATE_CLASS(MslSolver);
