...
TYPED_TEST(SoftmaxWithLossLayerTest, TestGradient) {
  ...
  SoftmaxWithLossLayer<Dtype, Dtype, Dtype> layer(layer_param);
...
TYPED_TEST(SoftmaxWithLossLayerTest, TestForwardIgnoreLabel) {
  ...
  scoped_ptr<SoftmaxWithLossLayer<Dtype, Dtype, Dtype> > layer(
      new SoftmaxWithLossLayer<Dtype, Dtype, Dtype>(layer_param));
  ...
  for (int_tp label = 0; label < 5; ++label) {
    ...
    layer.reset(new SoftmaxWithLossLayer<Dtype, Dtype, Dtype>(layer_param));
    layer->SetUp(this->blob_bottom_vec_, this->blob_top_vec_);
    layer->Forward(this->blob_bottom_vec_, this->blob_top_vec_);
  ...
  Dtype delta = std::is_same<Dtype, half_fp>::value ?
                1e-1 : 1e-4;
  EXPECT_NEAR(4 * full_loss, accum_loss, delta * fabs(accum_loss));
}
