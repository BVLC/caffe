...
TYPED_TEST(ReshapeLayerTest, TestSetup) {
  ...
  layer_param.set_num(1);
  layer_param.set_channels(2 * 3);
  layer_param.set_height(6);
  layer_param.set_widht(5);
  ...
  layer_param.set_num(2);
  layer_param.set_channels(3 * 6 * 5);
  layer_param.set_height(1);
  layer_param.set_widht(1);
  layer.reset(new ReshapeLayer<TypeParam>(layer_param));
  layer.SetUp(this->blob_bottom_vec_, &(this->blob_top_vec_));
    ...
    for (int h = 0; h < 6; ++h) {
      for (int w = 0; w < 5; ++w) {
        EXPECT_EQ(this->blob_top_->data_at(0, c, h, w),
          this->blob_bottom_->data_at(0, c % 2, h, w));
      }
    }
  }
}
