...
TYPED_TEST(LibDNNDeconvolutionLayerTest, TestNDAgainst2D) {
  ...
  convolution_param->mutable_weight_filler()->set_type("gaussian");
  Blob<TypeParam> weights;
  Blob<TypeParam> top_diff;
  ...
  Blob<TypeParam> result_2d;
  Blob<TypeParam> backward_result_2d;
  Blob<TypeParam> backward_weight_result_2d;
  // Test with 2D im2col
  {
    caffe_set(this->blob_top_->count(), TypeParam(0),
              this->blob_top_->mutable_cpu_data());
    caffe_set(this->blob_bottom_->count(), TypeParam(0),
              this->blob_bottom_->mutable_cpu_diff());
    caffe_set(weights.count(), TypeParam(0), weights.mutable_cpu_diff());
    // Do SetUp and Forward; save Forward result in result_2d.
    convolution_param->set_force_nd_im2col(false);
    LibDNNDeconvolutionLayer<TypeParam> layer_2d(layer_param);
  ...
  Blob<TypeParam> result_nd;
  Blob<TypeParam> backward_result_nd;
  Blob<TypeParam> backward_weight_result_nd;
  // Test with ND im2col
  {
    caffe_set(this->blob_top_->count(), TypeParam(0),
              this->blob_top_->mutable_cpu_data());
    ...
    LibDNNDeconvolutionLayer<TypeParam> layer_nd(layer_param);
