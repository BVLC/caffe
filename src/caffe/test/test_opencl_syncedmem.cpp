...
class OpenCLSyncedMemoryTest : public ::testing::Test {};

TEST_F(OpenCLSyncedMemoryTest, TestInitialization) {
  OpenCLSyncedMemory mem(10);
  EXPECT_EQ(mem.head(), OpenCLSyncedMemory::UNINITIALIZED);
  EXPECT_EQ(mem.size(), 10);
  OpenCLSyncedMemory* p_mem = new OpenCLSyncedMemory(10 * sizeof(float));
  EXPECT_EQ(p_mem->size(), 10 * sizeof(float));
  delete p_mem;
}

TEST_F(OpenCLSyncedMemoryTest, TestAllocation) {
  OpenCLSyncedMemory mem(10);
...
TEST_F(OpenCLSyncedMemoryTest, TestCPUWrite) {
  OpenCLSyncedMemory mem(10);
  void* cpu_data = mem.mutable_cpu_data();
  EXPECT_EQ(mem.head(), OpenCLSyncedMemory::HEAD_AT_CPU);
  memset(cpu_data, 1, mem.size());
  for (int i = 0; i < mem.size(); ++i) {
    EXPECT_EQ((reinterpret_cast<char*>(cpu_data))[i], 1);
  }
  const void* gpu_data = mem.gpu_data();
  EXPECT_EQ(mem.head(), OpenCLSyncedMemory::SYNCED);
...
TEST_F(OpenCLSyncedMemoryTest, TestGPUWrite) {
  OpenCLSyncedMemory mem(10);
  void* gpu_data = mem.mutable_gpu_data();
  EXPECT_EQ(mem.head(), OpenCLSyncedMemory::HEAD_AT_GPU);
