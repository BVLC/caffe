...
class Im2colLayerTest: public MultiDeviceTest<TypeParam> {
 ...
 protected:
    ...
    void Im2colLayerTestForwardPerformance(
        int num_images,
        int num_channels,
        int im_width,
        int im_height) {
      ...
      ConvolutionParameter* convolution_param =
          layer_param.mutable_convolution_param();    // NOLINT(*)
    ...
    void Im2colLayerTestForwardValidation(
        LayerParameter lp,
        int num_images,
        int num_channels,
        int im_width,
      ...
      LOG(INFO) << "TestForwardValidation() : images(" << num_images
                << ") channels(" << num_channels << ") height("
      ...
      DLOG(INFO) << "BBLOB(" << BTMnum << ", " << BTMchannels << ", "
                 << BTMheight << ", " << BTMwidth << ")";
      ...
      DLOG(INFO) << "TBLOB(" << TOPnum << ", " << TOPchannels << ", "
                 << TOPheight << ", " << TOPwidth << ")";
      pixel = 0;
      ...
      Dtype* cpu_ = reinterpret_cast<Dtype*>(malloc(numTopElements*sizeof(Dtype)));  // NOLINT(*)
      Dtype* gpu_ = reinterpret_cast<Dtype*>(malloc(numTopElements*sizeof(Dtype)));  // NOLINT(*)

      // run on CPU
      Caffe::set_mode(Caffe::CPU);
      layer.Forward(this->blob_bottom_vec_, this->blob_top_vec_);
      memcpy(cpu_, this->blob_top_->cpu_data(), numTopElements*sizeof(Dtype));  // NOLINT(*)

      // clear CPU result
      LOG(INFO) << "TBLOB(" << TOPnum << ", " << TOPchannels << ", "
                << TOPheight << ", " << TOPwidth << ")";
      pixel = 0;
      for (int n = 0; n < TOPnum; n++) {
        for (int c = 0; c < TOPchannels; c++) {
      ...
      layer.Forward(this->blob_bottom_vec_, this->blob_top_vec_);
      memcpy(gpu_, this->blob_top_->cpu_data(), numTopElements*sizeof(Dtype));  // NOLINT(*)
...
for (int im = 1; im <= 3; im++) {
  for (int ch = 1; ch <= 3; ch++) {
    for (int i = 0; i < 10; i++) {
      h = min + rand() / (RAND_MAX / (max - min + 1) + 1);  // NOLINT(*)
      w = min + rand() / (RAND_MAX / (max - min + 1) + 1);  // NOLINT(*)
