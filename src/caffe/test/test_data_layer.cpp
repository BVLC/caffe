...
class DataLayerTest : public MultiDeviceTest<TypeParam> {
 ...
 protected:
  DataLayerTest()
      : backend_(DatumDBParameter_Backend_LEVELDB),
  ...
  void Fill(const bool unique_pixels, DatumDBParameter_Backend backend) {
    backend_ = backend;
...
TYPED_TEST(DataLayerTest, TestReadLevelDB) {
  ...
  this->Fill(unique_pixels, DatumDBParameter_Backend_LEVELDB);
  this->TestRead();
}
...
TYPED_TEST(DataLayerTest, TestReadLMDB) {
  const bool unique_pixels = false;  // all pixels the same; images different
  this->Fill(unique_pixels, DatumDBParameter_Backend_LMDB);
  this->TestRead();
}
