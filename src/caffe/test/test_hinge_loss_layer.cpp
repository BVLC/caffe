...
class HingeLossLayerTest: public MultiDeviceTest<TypeParam> {
 ...
 protected:
    ...
    void HingeLossLayerTestSetup(
        int num_images,
        int num_channels,
        int im_width,
        int im_height) {
      blob_bottom_data_->Reshape(num_images, num_channels,
                                 im_height, im_width);
      blob_bottom_label_->Reshape(num_images, num_channels,
                                  im_height, im_width);
    ...
    void HingeLossLayerTestForwardPerformance(
        int num_images,
        int num_channels,
        int im_width,
        int im_height) {
      this->HingeLossLayerTestSetup(num_images, num_channels,
                                    im_width, im_height);
...
for (int i = TEST_IMAGE_WIDTH_MIN; i <= 1024; i*=2) {
  this->HingeLossLayerTestForwardPerformance(TEST_NUM_IMAGES, TEST_NUM_CHANNELS,
                                             i, i);
}
}
