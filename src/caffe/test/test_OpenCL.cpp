...
TYPED_TEST(OpenCLSimpleTest, TestMaxMemory) {
  ...
  void** vPtrArr = reinterpret_cast<void**>(malloc(max_cnt*sizeof(void *)));
...
TYPED_TEST(OpenCLSimpleTest, TestMaxBuffer) {
  ...
  int64_t megabytes= suc_cnt * alloc_increment / MB;
...
TYPED_TEST(OpenCLSimpleTest, TestMemcpy) {
  ...
  TypeParam* cpuPtrA = reinterpret_cast<TypeParam*>(malloc(bytes));
  EXPECT_TRUE(cpuPtrA != NULL);

  TypeParam* cpuPtrB = reinterpret_cast<TypeParam*>(malloc(bytes));
  EXPECT_TRUE(cpuPtrB != NULL);

  for (int i = 0; i < count; i++) {
    cpuPtrA[i] = rand() % count;  // NOLINT(*)
    cpuPtrB[i] = 0;
  }
  ...
  EXPECT_TRUE(caffe::OpenCL::clMemcpy(
      gpuPtrA, cpuPtrA, bytes, caffe::OpenCL::COPY_CPU_TO_GPU));
  EXPECT_TRUE(caffe::OpenCL::clMemcpy(
      gpuPtrB, gpuPtrA, bytes, caffe::OpenCL::COPY_GPU_TO_GPU));
  EXPECT_TRUE(caffe::OpenCL::clMemcpy(
      cpuPtrB, gpuPtrB, bytes, caffe::OpenCL::COPY_GPU_TO_CPU));
...
TYPED_TEST(OpenCLSimpleTest, TestMemcpyOffset) {
  ...
  EXPECT_TRUE(caffe::OpenCL::clMemcpy(
      gpuPtr, cpuPtrB, bytes, caffe::OpenCL::COPY_CPU_TO_GPU));
  EXPECT_TRUE(caffe::OpenCL::clMemcpy(
      static_cast<char*>(gpuPtr) + offsetInBytes,
      cpuPtrA+offset, bytes-offsetInBytes, caffe::OpenCL::COPY_CPU_TO_GPU));
  EXPECT_TRUE(caffe::OpenCL::clMemcpy(
      cpuPtrB, gpuPtr, bytes, caffe::OpenCL::COPY_GPU_TO_CPU));
...
TYPED_TEST(OpenCLSimpleTest, TestMemcpyOverlay) {
  ...
  EXPECT_TRUE(caffe::OpenCL::clMemcpy(
      gpuPtr, cpuPtrA, bytes, caffe::OpenCL::COPY_CPU_TO_GPU));
  EXPECT_TRUE(caffe::OpenCL::clMemcpy(
      gpuPtr, static_cast<char*>(gpuPtr) + offsetInBytes,
      bytes-offsetInBytes, caffe::OpenCL::COPY_GPU_TO_GPU));
  EXPECT_TRUE(caffe::OpenCL::clMemcpy(
      cpuPtrB, gpuPtr, bytes, caffe::OpenCL::COPY_GPU_TO_CPU));
...
TYPED_TEST(OpenCLSimpleTest, TestMemsetOffset) {
  ...
  EXPECT_TRUE(caffe::OpenCL::clMemcpy(
      gpuPtr, cpuPtrA, bytes, caffe::OpenCL::COPY_CPU_TO_GPU));
  EXPECT_TRUE(caffe::OpenCL::clMemset(
      static_cast<char*>(gpuPtr) + offsetInBytes, alpha, bytes-offsetInBytes));
  EXPECT_TRUE(caffe::OpenCL::clMemcpy(
      cpuPtrB, gpuPtr, bytes, caffe::OpenCL::COPY_GPU_TO_CPU));
  for (int i = offset; i < count; i++) {
    EXPECT_EQ(cpuPtrB[i], alpha);
  ...
  EXPECT_TRUE(caffe::OpenCL::clMemcpy(
      gpuPtr, cpuPtrA, bytes, caffe::OpenCL::COPY_CPU_TO_GPU));
  EXPECT_TRUE(caffe::OpenCL::clMemset(
      static_cast<char*>(gpuPtr) + offsetInBytes, beta, bytes-offsetInBytes));
  EXPECT_TRUE(caffe::OpenCL::clMemcpy(
      cpuPtrB, gpuPtr, bytes, caffe::OpenCL::COPY_GPU_TO_CPU));
  for (int i = offset; i < count; i++) {
    EXPECT_EQ(cpuPtrB[i], beta);
...
TYPED_TEST(OpenCLSimpleTest, Test_clBLASgemv) {
  ...
  TypeParam* cpuPtr_mxn = reinterpret_cast<TypeParam*>(mem_mxn.mutable_cpu_data());  // NOLINT(*)
  EXPECT_TRUE(cpuPtr_mxn != NULL);

  int idx = 0;
  for (int i = 0; i < m; i++) {
    for (int j = 0; j < n; j++) {
      cpuPtr_mxn[idx] = rand() % (m*n);  // NOLINT(*)
      idx++;
    }
  }

  TypeParam* gpuPtr_mxn  = reinterpret_cast<TypeParam*>(mem_mxn.mutable_gpu_data());  // NOLINT(*)
  ...
  TypeParam* cpuPtr_n = reinterpret_cast<TypeParam*>(mem_n.mutable_cpu_data());
  EXPECT_TRUE(cpuPtr_n != NULL);

  for (int i = 0; i < n; i++) {
    cpuPtr_n[i] = rand() % (m*n);  // NOLINT(*)
  }
  // SNAPSHOT(cpuPtr_n, n);

  TypeParam* gpuPtr_n  = reinterpret_cast<TypeParam*>(mem_n.mutable_gpu_data());
  ...
  TypeParam* cpuPtr_m = reinterpret_cast<TypeParam*>(mem_m.mutable_cpu_data());
  EXPECT_TRUE(cpuPtr_m != NULL);

  for (int i = 0; i < m; i++) {
    cpuPtr_m[i] = rand() % (m*n);  // NOLINT(*)
  }
  // SNAPSHOT(cpuPtr_m, m);

  TypeParam* gpuPtr_m  = reinterpret_cast<TypeParam*>(mem_m.mutable_gpu_data());
  EXPECT_TRUE(gpuPtr_m != NULL);

  TypeParam alpha = 0.25;
  TypeParam beta  = 0.0;

  EXPECT_TRUE(caffe::OpenCL::clBLASgemv(clblasNoTrans, m, n, alpha, gpuPtr_mxn,
                                        gpuPtr_n, beta, gpuPtr_m));
  mem_m.cpu_data();
  // SNAPSHOT(cpuPtr_m, m);
  ...
  EXPECT_TRUE(caffe::OpenCL::clBLASgemv(clblasTrans, m, n, alpha, gpuPtr_mxn,
                                        gpuPtr_m, beta, gpuPtr_n));
  mem_n.cpu_data();
  // SNAPSHOT(cpuPtr_n, n);
...
TYPED_TEST(OpenCLSimpleTest, Test_clBLASdot) {
  ...
  TypeParam* cpuPtr_x = reinterpret_cast<TypeParam*>(mem_x.mutable_cpu_data());
  EXPECT_TRUE(cpuPtr_x != NULL);

  for (int i = 0; i < n; i++) {
    cpuPtr_x[i] = rand() % (n);  // NOLINT(*)
  }
  // SNAPSHOT(cpuPtr_x, n);

  TypeParam* gpuPtr_x  = reinterpret_cast<TypeParam*>(mem_x.mutable_gpu_data());
  EXPECT_TRUE(gpuPtr_x != NULL);

  // create [x] vector
  SyncedMemory mem_y(n_size);
  TypeParam* cpuPtr_y = reinterpret_cast<TypeParam*>(mem_y.mutable_cpu_data());
  EXPECT_TRUE(cpuPtr_y != NULL);

  for (int i = 0; i < n; i++) {
    cpuPtr_y[i] = rand() % (n);  // NOLINT(*)
  }
  // SNAPSHOT(cpuPtr_y, n);

  TypeParam* gpuPtr_y  = reinterpret_cast<TypeParam*>(mem_y.mutable_gpu_data());
...
TYPED_TEST(OpenCLSimpleTest, Test_clsub) {
  ...
  TypeParam* cpuPtr_z = reinterpret_cast<TypeParam*>(mem_z.mutable_cpu_data());
  ...
  TypeParam* gpuPtr_z  = reinterpret_cast<TypeParam*>(mem_z.mutable_gpu_data());
