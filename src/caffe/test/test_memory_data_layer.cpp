...
class MemoryDataLayerTest: public MultiDeviceTest<TypeParam> {
 ...
 protected:
    ...
    void MemoryDataLayerTestSetup(
      ...
      FillerParameter filler_param;
      GaussianFiller<Dtype> filler(filler_param);
      data_->Reshape(num_batches * batch_size, num_channels,
                     im_height, im_width);
    ...
    void MemoryDataLayerTestForwardPerformance(
        int num_batches,
        int batch_size,
        int num_channels,
        int im_width,
        int im_height) {
      this->MemoryDataLayerTestSetup(num_batches, batch_size, num_channels,
                                     im_width, im_height);
      ...
      shared_ptr<MemoryDataLayer<Dtype> > layer(new MemoryDataLayer<Dtype>(layer_param));   // NOLINT(*)
      layer->DataLayerSetUp(this->blob_bottom_vec_, this->blob_top_vec_);
      layer->Reset(this->data_->mutable_cpu_data(), this->labels_->mutable_cpu_data(), this->data_->num());  // NOLINT(*)
