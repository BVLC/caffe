...
class DetectionEvaluateLayerTest : public CPUDeviceTest<Dtype> {
 protected:
  ...
  void FillData() {
    // Fill ground truth.
    FillItem(blob_bottom_gt_, 0, "0 1 0 0.1 0.1 0.3 0.3");
    FillItem(blob_bottom_gt_, 1, "0 1 0 0.6 0.6 0.8 0.8");
    FillItem(blob_bottom_gt_, 2, "1 2 0 0.3 0.3 0.6 0.5");
    FillItem(blob_bottom_gt_, 3, "1 1 0 0.7 0.1 0.9 0.3");

    // Fill detections.
    FillItem(blob_bottom_det_, 0, "0 1 0.3 0.1 0.0 0.4 0.3");
    FillItem(blob_bottom_det_, 1, "0 1 0.7 0.0 0.1 0.2 0.3");
    FillItem(blob_bottom_det_, 2, "0 1 0.9 0.7 0.6 0.8 0.8");
    FillItem(blob_bottom_det_, 3, "1 2 0.8 0.2 0.1 0.4 0.4");
    FillItem(blob_bottom_det_, 4, "1 2 0.1 0.4 0.3 0.7 0.5");
    FillItem(blob_bottom_det_, 5, "1 1 0.2 0.8 0.1 1.0 0.3");
    FillItem(blob_bottom_det_, 6, "1 3 0.2 0.8 0.1 1.0 0.3");
    FillItem(blob_bottom_det_, 7, "2 1 0.2 0.8 0.1 1.0 0.3");
  }
  ...
  void FillItem(Blob<Dtype>* blob, const int item, const string values) {
    ...
    for (int i = 0; i < 2; ++i) {
      blob_data[item * 7 + i] = atoi(items[i].c_str());
    }
    for (int i = 2; i < 7; ++i) {
      blob_data[item * 7 + i] = atof(items[i].c_str());
    }
  }
