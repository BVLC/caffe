...
void ConvolutionLayerFFT<Dtype>::fft_gpu_setup() {
  ...
  size_t fft_gpu_map_in_complex_bytes =
      fft_map_complex_size_ * sizeof(DtypeComplex<Dtype>);
  ...
  fft_gpu_weights_complex_reshape_ = state.create_buffer(CL_MEM_READ_WRITE,
      fft_gpu_weights_complex_bytes, NULL);
#endif

  fft_gpu_map_in_real_all_channels_ = state.create_buffer(CL_MEM_READ_WRITE,
      fft_gpu_map_in_real_bytes * this->channels_, NULL);
  fft_gpu_map_in_complex_all_channels_ = state.create_buffer(CL_MEM_READ_WRITE,
      fft_gpu_map_in_complex_bytes * this->channels_, NULL);

  fft_gpu_map_in_real_all_num_output_ = state.create_buffer(CL_MEM_READ_WRITE,
      fft_gpu_map_in_real_bytes * this->num_output_, NULL);
  fft_gpu_map_in_complex_all_num_output_ =
      state.create_buffer(CL_MEM_READ_WRITE,
      fft_gpu_map_in_complex_bytes * this->num_output_, NULL);

  fft_gpu_map_out_complex_ = state.create_buffer(CL_MEM_READ_WRITE,
      fft_gpu_map_out_complex_bytes, NULL);
  fft_gpu_map_out_real_ = state.create_buffer(CL_MEM_READ_WRITE,
      fft_gpu_map_out_real_bytes, NULL);
...
void ConvolutionLayerFFT<Dtype>::fft_gpu_clean() {
  if (fft_gpu_initialized_) {
    ClState& state = Caffe::cl_state();
    state.destroy_buffer(fft_gpu_weights_complex_);
#ifdef COMPLEX_NULT_CONJ_RESHAPE
    state.destroy_buffer(fft_gpu_weights_complex_reshape_);
#endif
    state.destroy_buffer(fft_gpu_map_in_real_all_channels_);
    state.destroy_buffer(fft_gpu_map_in_complex_all_channels_);
    state.destroy_buffer(fft_gpu_map_in_real_all_num_output_);
    state.destroy_buffer(fft_gpu_map_in_complex_all_num_output_);
    state.destroy_buffer(fft_gpu_map_out_complex_);
    state.destroy_buffer(fft_gpu_map_out_real_);
  }
...
void ConvolutionLayerFFT<Dtype>::fft_gpu_compute_weights() {
  int num_weights = this->num_output_ * (this->channels_ / this->group_);
  // Clear buffer
  clear_gpu_fft_buffer(fft_gpu_weights_complex_,
      num_weights * fft_map_complex_size_ * sizeof(DtypeComplex<Dtype>));
...
void ConvolutionLayerFFT<Dtype>::Backward_gpu_fft_task(
    const vector<Blob<Dtype>*>& bottom, const vector<Blob<Dtype>*>& top,
    const Dtype* weight, int i, int n, int ch_gr, int out_gr) {
