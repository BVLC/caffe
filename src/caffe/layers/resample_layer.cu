...
__global__ void ResampleForward(const int nthreads, const Dtype* bottom_data,
  ...
  CUDA_KERNEL_LOOP(index, nthreads) {
    ...
    top_data[index] = bottom_data[sampled_index];
    if (mask) {
...
__global__ void ResampleForwardWindowed(const int nthreads, const Dtype* bottom_data,
  ...
  CUDA_KERNEL_LOOP(index, nthreads) {
    ...
    int wstart = bottom_window_data[win * 4] * width / image_w;
    int hstart = bottom_window_data[win * 4 + 1] * height / image_h;
    int window_w = max((int)ceil(bottom_window_data[win * 4 + 2] * width * 1.0 / image_w), 1);
    int window_h = max((int)ceil(bottom_window_data[win * 4 + 3] * height * 1.0 / image_h), 1);
    ...
    if (mask) {
      ...
      for (int win = 0; win < window_count; ++win) {
        ...
        for (int pool = 0; pool < num_output; ++pool) {
          if (mask[win * num_output + pool] == h * width + w) {
            gradient += top_diff[win * num_output + pool];
          }
        }
      }
    } else {
      ...
      for (int win = 0; win < window_count; ++win) {
        // Because of crazy indexing in windows, easier to check every window.
        for (int pool = 0; pool < num_output; ++pool) {
          if (top_mask[win * num_output + pool] == h * width + w) {
            gradient += top_diff[win * num_output + pool];
          }
        }
      }
