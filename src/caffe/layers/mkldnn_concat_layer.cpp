...
//    VLOG(1) << "MKLDNNConcatLayer<Dtype>::LayerSetUp: " << this->layer_param_.name();
    ...
    split_channels = new int[num_concats_];
                                          ...
                                          const vector<Blob<Dtype>*>& top)
{
    ...
    for (auto i = 0; i < num_concats_; i++) {
        ...
        input_primitive.push_back(*(fwd_bottom_data[i]->create_input(false)));
    }
...
// FIXME: concat dimension
    ...
    fwd_top_data.reset(new MKLDNNData<Dtype>(usr_dst_mpd, prv_dst_mpd, top[0], this));
    output_memory = fwd_top_data->create_output_memory();

    concatFwd.reset(new concat(*concatFwd_pd, input_primitive, *output_memory));
