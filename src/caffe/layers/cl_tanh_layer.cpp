...
void TanHLayer<Dtype>::Forward_gpu(const vector<Blob<Dtype>*>& bottom,
  ...
  state.submit_program("tanh", &_cl_tanh_layer_start, &_cl_tanh_layer_end);

  // Compute inner1d(top_diff, top_data) and subtract them from the bottom diff
  ClKernel kernel = state.get_kernel("TanHForward");
...
void TanHLayer<Dtype>::Backward_gpu(const vector<Blob<Dtype>*>& top,
  ...
  if (propagate_down[0]) {
      ...
      //   bottom diff
      ClKernel kernel = state.get_kernel("TanHBackward");
      kernel.set_arg(argIdx++, count);
