...
template <class T> __kernel void LRNFillScale(const int nthreads, const global T* in, const int num, const int channels, const int height, const int width, const int size, const T alpha_over_size, global T* scale) {
...
template __attribute__((mangled_name(LRNFillScaleFloat))) kernel void LRNFillScale(const int nthreads, const global float* in, const int num, const int channels, const int height, const int width, const int size, const float alpha_over_size, global float* scale);
template __attribute__((mangled_name(LRNFillScaleDouble))) kernel void LRNFillScale(const int nthreads, const global double* in, const int num, const int channels, const int height, const int width, const int size, const double alpha_over_size, global double* scale);
...
template <class T> __kernel void LRNComputeDiff(const int nthreads, const global T* bottom_data, const global T* top_data, const global T* scale, const global T* top_diff, const int num, const int channels, const int height, const int width, const int size, const T negative_beta, const T cache_ratio, global T* bottom_diff) {
	...
	if ( idx < nthreads ) {
		...
		while (head < size) {
			accum_ratio += top_diff[head * step] * top_data[head * step] / scale[head * step];
			bottom_diff[(head - post_pad) * step] = top_diff[(head - post_pad) * step] * pow(scale[(head - post_pad) * step], negative_beta) - cache_ratio * bottom_data[(head - post_pad) * step] * accum_ratio;
			++head;
		}
		...
		while (head < channels) {
			accum_ratio += top_diff[head * step] * top_data[head * step] /scale[head * step];
			accum_ratio -= top_diff[(head - size) * step] * top_data[(head - size) * step] / scale[(head - size) * step];
			bottom_diff[(head - post_pad) * step] = top_diff[(head - post_pad) * step] * pow(scale[(head - post_pad) * step], negative_beta) - cache_ratio * bottom_data[(head - post_pad) * step] * accum_ratio;
			++head;
		}
