...
void BatchNormLayer<Dtype>::Reshape(const vector<Blob<Dtype>*>& bottom,
  ...
  top_data_cache_.ReshapeLike(*bottom[0]);
  sz[0]=bottom[0]->shape(0);
  num_sum_.Reshape(sz);

  int spatial_dim = bottom[0]->count()/(channels_*bottom[0]->shape(0));
  if (sum_multiplier_.num_axes() == 0 ||
      sum_multiplier_.shape(0) != spatial_dim) {
    sz[0] = spatial_dim;
    sum_multiplier_.Reshape(sz);
...
void BatchNormLayer<Dtype>::Forward_cpu(const vector<Blob<Dtype>*>& bottom,
  ...
  if (use_global_stats_) {
    ...
    caffe_cpu_gemv<Dtype>(CblasNoTrans, channels_ * num, spatial_dim,
        1. / (num * spatial_dim), bottom_data, sum_multiplier_.cpu_data(), 0.,
        num_by_chans_.mutable_cpu_data());
    caffe_cpu_gemv<Dtype>(CblasTrans, num, channels_, 1.,
        num_by_chans_.cpu_data(), num_sum_.cpu_data(), 0.,
        mean_.mutable_cpu_data());
    caffe_cpu_gemv<Dtype>(CblasNoTrans, channels_ * num, spatial_dim,
        1. / (num * spatial_dim), temp_.cpu_data(), sum_multiplier_.cpu_data(),
  ...
  caffe_cpu_gemm<Dtype>(CblasNoTrans, CblasNoTrans, num, channels_, 1, 1,
      num_sum_.cpu_data(), mean_.cpu_data(), 0.,
      num_by_chans_.mutable_cpu_data());
  caffe_cpu_gemm<Dtype>(CblasNoTrans, CblasNoTrans, channels_ * num,
      spatial_dim, 1, -1, num_by_chans_.cpu_data(), sum_multiplier_.cpu_data(),
      1., top_data);
  // replicate variance to input size
  caffe_cpu_gemm<Dtype>(CblasNoTrans, CblasNoTrans, num, channels_, 1, 1,
      num_sum_.cpu_data(), variance_.cpu_data(), 0.,
      num_by_chans_.mutable_cpu_data());
  caffe_cpu_gemm<Dtype>(CblasNoTrans, CblasNoTrans, channels_ * num,
      spatial_dim, 1, 1., num_by_chans_.cpu_data(), sum_multiplier_.cpu_data(),
  ...
  caffe_copy(top_data_cache_.count(), top_data,
      top_data_cache_.mutable_cpu_data());
}
...
void BatchNormLayer<Dtype>::Backward_cpu(const vector<Blob<Dtype>*>& top,
  ...
  const Dtype* top_data = top_data_cache_.cpu_data();
  ...
  caffe_cpu_gemv<Dtype>(CblasNoTrans, channels_ * num, spatial_dim, 1.,
      bottom_diff, sum_multiplier_.cpu_data(), 0.,
      num_by_chans_.mutable_cpu_data());
  caffe_cpu_gemv<Dtype>(CblasTrans, num, channels_, 1.,
      num_by_chans_.cpu_data(), num_sum_.cpu_data(), 0.,
      mean_.mutable_cpu_data());
  ...
  caffe_cpu_gemv<Dtype>(CblasNoTrans, channels_ * num, spatial_dim, 1.,
      top_diff, sum_multiplier_.cpu_data(), 0.,
      num_by_chans_.mutable_cpu_data());
  caffe_cpu_gemv<Dtype>(CblasTrans, num, channels_, 1.,
      num_by_chans_.cpu_data(), num_sum_.cpu_data(), 0.,
      mean_.mutable_cpu_data());
