...
void MemoryDataLayer<Dtype>::DataLayerSetUp(const vector<Blob<Dtype>*>& bottom,
  ...
  added_sample_weight_.Reshape(batch_size_, 1, 1, 1);
  data_ = NULL;
  labels_ = NULL;
  sample_weights_ = NULL;
  added_data_.cpu_data();
  added_label_.cpu_data();
  added_sample_weight_.cpu_data();
}
...
void MemoryDataLayer<Dtype>::AddDatumVector(const vector<Datum>& datum_vector) {
  ...
  Dtype* top_sample_weight = added_sample_weight_.mutable_cpu_data();
  for (int batch_item_id = 0; batch_item_id < num; ++batch_item_id) {
    ...
    top_sample_weight[batch_item_id] = datum_vector[batch_item_id].sample_weight();
  }
  // num_images == batch_size_
  Reset(top_data, top_label, top_sample_weight, batch_size_);
  has_new_data_ = true;
}

template <typename Dtype>
void MemoryDataLayer<Dtype>::Reset(Dtype* data, Dtype* labels, Dtype* sample_weights, int n) {
  CHECK(data);
  CHECK(labels);
...
void MemoryDataLayer<Dtype>::Forward_cpu(const vector<Blob<Dtype>*>& bottom,
  ...
  (*top)[2]->set_cpu_data(sample_weights_ + pos_);
