...
template bool clMaxPoolBackward<float>(
    const int nthreads,
    const float* top_diff,
    const int* mask,
    const float* top_mask,
    const int num,
    const int channels,
    const int height,
    const int width,
    ...
    const int pad_w,
    float* bottom_diff);
template bool clMaxPoolBackward<double>(
    const int nthreads,
    const double* top_diff,
    const int* mask,
    const double* top_mask,
    const int num,
    const int channels,
    const int height,
    const int width,
...
template bool clAvePoolBackward<float>(
    const int nthreads,
    const float* top_diff,
    const int num,
    const int channels,
    ...
    const int pad_w,
    float* bottom_diff);
template bool clAvePoolBackward<double>(
    const int nthreads,
    const double* top_diff,
    const int num,
    const int channels,
...
template bool clStoPoolBackward<float>(
    ...
    const int stride_w,
    float* bottom_diff);
template bool clStoPoolBackward<double>(
    const int nthreads,
...
bool clMaxPoolForward(
  ...
  size_t global3D[3] = {
      CAFFE_GET_GLOBAL_WORKITEMS(pooled_width, OPENCL_LOCAL_SIZE),
      (size_t) pooled_height, (size_t) num * channels };
  size_t local3D[3] = { OPENCL_LOCAL_SIZE, 1, 1 };

  size_t global1D = nthreads;
  // CAFFE_GET_GLOBAL_WORKITEMS(nthreads, OPENCL_LOCAL_SIZE);
  size_t local1D  = 1;
  // CAFFE_GET_LOCAL_WORKITEMS(nthreads, OPENCL_LOCAL_SIZE);
...
template bool clAvePoolForward<float>(
    const int nthreads,
    const float* bottom_data,
    const int num,
    const int channels,
    ...
    const int pad_w,
    float* top_data);
template bool clAvePoolForward<double>(
    const int nthreads,
    const double* bottom_data,
    const int num,
    const int channels,
...
template bool clStoPoolForwardTrain<float>(
    const int nthreads,
    const float* bottom_data,
    const int num,
    const int channels,
    ...
    float* rand_idx,
    float* top_data);
template bool clStoPoolForwardTrain<double>(
    const int nthreads,
    const double* bottom_data,
    const int num,
    const int channels,
...
template bool clStoPoolForwardTest<float>(
    const int nthreads,
    const float* bottom_data,
    const int num,
    const int channels,
    ...
    const int stride_w,
    float* top_data);
template bool clStoPoolForwardTest<double>(
    const int nthreads,
    const double* bottom_data,
    const int num,
    const int channels,
  ...
  switch (this->layer_param_.pooling_param().pool()) {
    case PoolingParameter_PoolMethod_MAX:
      ...
      TIME("caffe::OpenCL::clMaxPoolForward()", {
          BOOL_CHECK(
              caffe::OpenCL::clMaxPoolForward(
                  count, bottom_data, bottom[0]->num(),
    ...
    case PoolingParameter_PoolMethod_STOCHASTIC:
      if (this->phase_ == TRAIN) {
        ...
        caffe_gpu_rng_uniform(count, Dtype(0), Dtype(1), rand_idx_.mutable_gpu_data());  // NOLINT(*)
        /*
         // NOLINT_NEXT_LINE(whitespace/operators)
         ...
         */
        BOOL_CHECK(
            caffe::OpenCL::clStoPoolForwardTest(
                count, bottom_data, bottom[0]->num(),
                channels_, height_, width_, pooled_height_, pooled_width_,
                kernel_h_, kernel_w_, stride_h_, stride_w_, top_data));
      }
      break;
    default:
      LOG(FATAL)<< "Unknown pooling method.";
      ...
      TIME("caffe::OpenCL::clMaxPoolBackward", {
          BOOL_CHECK(
              caffe::OpenCL::clMaxPoolBackward<Dtype>(
                  count, top_diff, mask, top_mask, top[0]->num(),
                  channels_, height_, width_, pooled_height_, pooled_width_,
                  kernel_h_, kernel_w_, stride_h_, stride_w_, pad_h_, pad_w_,
                  bottom_diff));
      });
    ...
    case PoolingParameter_PoolMethod_STOCHASTIC:
      ...
      BOOL_CHECK(
          caffe::OpenCL::clStoPoolBackward<Dtype>(
              ...
              channels_, height_, width_, pooled_height_, pooled_width_,
              kernel_h_, kernel_w_, stride_h_, stride_w_, bottom_diff));
      break;
    default:
      LOG(FATAL)<< "Unknown pooling method.";
    }
