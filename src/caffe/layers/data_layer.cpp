...
void DataLayer<Dtype>::DataLayerSetUp(const vector<Blob<Dtype>*>& bottom,
  ...
  if (this->untransformed_top_)
    this->untransformed_data_.Reshape(top_shape);
  ...
  if (this->untransformed_top_)
    {
      for (int i = 0; i < this->PREFETCH_COUNT; ++i) {
        this->prefetch_untransformed_[i].data_.Reshape(top_shape);
      }
    }
  ...
  if (this->output_labels_) {
    ...
    if (this->untransformed_top_)
      for (int i = 0; i < this->PREFETCH_COUNT; ++i) {
        this->prefetch_untransformed_[i].label_.Reshape(label_shape);
      }
...
// This function is called on prefetch thread
template<typename Dtype>
void DataLayer<Dtype>::load_batch_and_untransformed_batch(Batch<Dtype>* batch, Batch<Dtype>* batch_untransformed) {
  CPUTimer batch_timer;
  batch_timer.Start();
  double read_time = 0;
  double trans_time = 0;
  ...
  if (this->untransformed_top_)
    {
      CHECK(batch_untransformed->data_.count());
      CHECK(this->untransformed_data_.count());
    }
  ...
  batch->data_.Reshape(top_shape);
  if (this->untransformed_top_)
    batch_untransformed->data_.Reshape(top_shape);
  ...
  if (this->output_labels_) {
    top_label = batch->label_.mutable_cpu_data();
    if (this->untransformed_top_)
      untransformed_top_label = batch_untransformed->label_.mutable_cpu_data();
  }
  for (int item_id = 0; item_id < batch_size; ++item_id) {
    timer.Start();
    // get a datum
    ...
    if (this->output_labels_) {
      top_label[item_id] = datum.label();
      if (this->untransformed_top_)
        untransformed_top_label[item_id] = datum.label();
    }
    trans_time += timer.MicroSeconds();
