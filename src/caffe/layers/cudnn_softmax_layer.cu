...
template <typename Dtype>
void CuDNNSoftmaxLayer<Dtype>::Forward_const_gpu(const vector<Blob<Dtype> *> &bottom,
  ...
  if (!bottom_desc_ptr_.get() || !top_desc_ptr_.get()) {
    auto softmax_axis_ = bottom[0]->CanonicalAxisIndex(
