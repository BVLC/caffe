...
bool clTanHLayerForward(const int count, const T* bottom_data, T* top_data) {
  ...
  cl_kernel* kernel = device.getKernel(
      kernel_name);
  if (kernel == NULL) {
    return false;
  }
...
template bool clTanHLayerForward<float>(
    const int count,
    const float* bottom_data,
    float* top_data);
template bool clTanHLayerForward<double>(
    const int count,
    const double* bottom_data,
    double* top_data);

template<typename T>
bool clTanHLayerBackward(
    const int count,
    const T* top_diff,
    const T* top_data,
    T* bottom_diff) {
...
template bool clTanHLayerBackward<float>(
    const int count,
    const float* top_diff,
    const float* top_data,
    float* bottom_diff);
template bool clTanHLayerBackward<double>(
    const int count,
...
void TanHLayer<Dtype>::Forward_gpu(
   ...
   */
  BOOL_CHECK(
      caffe::OpenCL::clTanHLayerForward(
          count,
          bottom_data,
          top_data));
}
...
void TanHLayer<Dtype>::Backward_gpu(
  ...
  if (propagate_down[0]) {
     ...
     */
    BOOL_CHECK(
        caffe::OpenCL::clTanHLayerBackward(
            count,
            top_diff,
            top_data,
            bottom_diff));
