      ...
      const vector<Blob<Dtype>*>& top)
{
  ...
  for ( size_t i = 0; i < num_tops; ++i) {
    ...
    e = dnnLayoutCreate<Dtype>(&(bwd_top_diff[i]->layout_usr), dim_src, sizes_src, strides_src);
  ...
  e = dnnLayoutCreate<Dtype>(&bwd_bottom_diff->layout_usr, dim_src, sizes_src, strides_src);
  CHECK_EQ(e, E_SUCCESS);
}
...
void MKLSplitLayer<Dtype>::Backward_cpu(const vector<Blob<Dtype>*>& top,
  ...
  bool num_prv = 0;
  for (size_t i = 0; i < num_tops; i++) {
    top_diff.push_back((void*)top[i]->prv_diff());
    if (top_diff[i] != NULL) {
      num_prv += 1;
    } else {
      top_diff[i] = (void*)top[i]->cpu_diff();
    }
  }
    ...
    if (sumPrimitive == NULL) {
      ...
      for (size_t i = 0; i < num_tops; ++i) {
        if ( top[i]->prv_diff() != NULL) {
          CHECK((top[i]->get_prv_descriptor_diff())->get_descr_type() == PrvMemDescr::PRV_DESCR_MKL2017);
          shared_ptr<MKLDiff<Dtype> > mem_descr =
            boost::static_pointer_cast<MKLDiff<Dtype> > (top[i]->get_prv_descriptor_diff());
      ...
      e = dnnSumCreate<Dtype>(&sumPrimitive, NULL, num_tops, int_layout, &coeffs_[0]);
      CHECK_EQ(e, E_SUCCESS);

      e = dnnLayoutCreateFromPrimitive<Dtype>(&bwd_bottom_diff->layout_int, sumPrimitive, dnnResourceDst);
      CHECK_EQ(e, E_SUCCESS);
      ...
      for (size_t i = 0; i < num_tops; ++i) {
        if ( top[i]->prv_diff() == NULL)
        {
          e = dnnLayoutCreateFromPrimitive<Dtype>(&bwd_top_diff[i]->layout_int, sumPrimitive, (dnnResourceType_t)(dnnResourceMultipleSrc + i));
          CHECK_EQ(e, E_SUCCESS);
  ...
  {
    if (sumPrimitive == NULL) {
      e = dnnSumCreate<Dtype>(&sumPrimitive, NULL, num_tops, bwd_bottom_diff->layout_usr, &coeffs_[0]);
      CHECK_EQ(e, E_SUCCESS);
    }
  }
  ...
  for (int i = 0; i < num_tops; ++i) {
    if (bwd_top_diff[i]->convert_to_int ) {
      sum_res[dnnResourceMultipleSrc + i] = bwd_top_diff[i]->get_converted_prv(top[i], false);
    } else {
      sum_res[dnnResourceMultipleSrc + i] = (void*)top_diff[i];
    }
  }

  if (bwd_bottom_diff->convert_from_int) {
    bottom[0]->set_prv_diff(bwd_bottom_diff->internal_ptr, bwd_bottom_diff, false);
    sum_res[dnnResourceDst] = (void*)bwd_bottom_diff->internal_ptr;
  } else {
    sum_res[dnnResourceDst] = (void*)(bottom[0]->mutable_cpu_diff());
  }
