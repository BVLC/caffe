...
void ConcatLayer<Dtype>::LayerSetUp(
...
template<typename Dtype>
void ConcatLayer<Dtype>::Forward_cpu(
    const vector<Blob<Dtype>*>& bottom,
    const vector<Blob<Dtype>*>& top) {
  ...
  for (int i = 0; i < bottom.size(); ++i) {
    ...
    for (int n = 0; n < num_concats_; ++n) {
      caffe_copy(bottom_concat_axis * concat_input_size_, bottom_data
          + n * bottom_concat_axis * concat_input_size_, top_data
...
template<typename Dtype>
void ConcatLayer<Dtype>::Backward_cpu(
    const vector<Blob<Dtype>*>& top,
  ...
  for (int i = 0; i < bottom.size(); ++i) {
    ...
    for (int n = 0; n < num_concats_; ++n) {
      caffe_copy(
          ...
          top_diff + (n * top_concat_axis + offset_concat_axis) * concat_input_size_,  // NOLINT(*)
...
template<typename Dtype>
void ConcatLayer<Dtype>::Forward_gpu(
    const vector<Blob<Dtype>*>& bottom,
    const vector<Blob<Dtype>*>& top) {
...
template<typename Dtype>
void ConcatLayer<Dtype>::Backward_gpu(
    const vector<Blob<Dtype>*>& top,
    const vector<bool>& propagate_down,
    const vector<Blob<Dtype>*>& bottom) {
