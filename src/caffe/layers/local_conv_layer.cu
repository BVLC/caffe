...
/*
template <typename Dtype>
void LocalConvolutionLayer<Dtype>::realign_bottom_diff_gpu(const Dtype
*loc_bottom_diff_buffer, Dtype *bottom_diff)
{
        //We are going to launch channels * loc_region_h * loc_region_w kernels,
each kernel responsible for
  ...
  realign_bottoom_diff_kernel<Dtype><<<CAFFE_GET_BLOCKS(num_kernels),
CAFFE_CUDA_NUM_THREADS >>>( num_kernels , loc_bottom_diff_buffer , conv_input_h,
conv_input_w , this->local_region_num_h_, this->local_region_num_w_ ,
conv_input_c , this->bottom_height_, this->bottom_width_ ,
