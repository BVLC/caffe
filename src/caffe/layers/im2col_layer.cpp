...
template<typename Dtype>
void Im2colLayer<Dtype>::Reshape(
    const vector<Blob<Dtype>*>& bottom,
    const vector<Blob<Dtype>*>& top) {
  ...
  DLOG(INFO) << "BTM shape = ( " << bottom[0]->num() << ", "
             << channels_ << ", " << height_ << ", " << width_ << " )";
  DLOG(INFO) << "TOP shape = ( " << bottom[0]->num() << ", "
...
bool Im2colLayer<Dtype>::setupMaskIM2COL() {
  if (height_ * width_ <= 0) {
    LOG(WARNING) << "skipping setup on height_ = "
...
template<typename Dtype>
void Im2colLayer<Dtype>::Forward_cpu(
    const vector<Blob<Dtype>*>& bottom,
    const vector<Blob<Dtype>*>& top) {
...
template<typename Dtype>
void Im2colLayer<Dtype>::Backward_cpu(
    const vector<Blob<Dtype>*>& top,
...
void Im2colLayer<Dtype>::Forward_gpu(
  ...
  if (this->hasKernelOverlap()) {
    DLOG(INFO) << "KernelOverlap = " << Im2colLayer<Dtype>::getHKernelOverlap()
               << " rows and " << Im2colLayer<Dtype>::getWKernelOverlap()
               << " cols";
  }
  ...
  switch (level) {
    case 1: {
      ...
      im2col_group_gpu(bottom_data, bottom_step, bottom[0]->num(),
                       channels_, height_, width_, kernel_h_, kernel_w_,
                       pad_h_, pad_w_, stride_h_, stride_w_,
                       top_data, top_step);
      ...
      /*
      im2col_group_gpu(bottom_data, this->im2col_mask_.gpu_data(),
                          bottom[0]->num(), channels_, height_, width_,
                          kernel_h_, kernel_w_, height_out_, width_out_,
                          top_data);
                          */
    }
      break;
    default:
      for (int n = 0; n < bottom[0]->num(); ++n) {
        ...
        /*
        im2col_gpu(bottom_data + bottom[0]->offset(n), channels_,
...
template<typename Dtype>
void Im2colLayer<Dtype>::Backward_gpu(
    const vector<Blob<Dtype>*>& top,
    const vector<bool>& propagate_down,
    const vector<Blob<Dtype>*>& bottom) {
  ...
  switch (OPENCL_OPT_LEVEL) {
    case 1: {
      int bottom_step = (bottom)[0]->offset(1);
      int top_step = top[0]->offset(1);
      col2im_gpu(top_diff, top_step, top[0]->num(), channels_, height_, width_,
