...
template bool clSigmoidLayerForward<float>(
    const int count,
    const float* bottom_data,
    float* top_data);
template bool clSigmoidLayerForward<double>(
    const int count,
    const double* bottom_data,
    double* top_data);

template<typename T>
bool clSigmoidLayerBackward(
    const int count,
    const T* top_diff,
    const T* top_data,
    T* bottom_diff) {
...
template bool clSigmoidLayerBackward<float>(
    const int count,
    const float* top_diff,
    const float* top_data,
    float* bottom_diff);
template bool clSigmoidLayerBackward<double>(
    const int count,
...
void SigmoidLayer<Dtype>::Forward_gpu(
  ...
  const int count = bottom[0]->count();
  BOOL_CHECK(
      caffe::OpenCL::clSigmoidLayerForward(count, bottom_data, top_data));
}

template<typename Dtype>
void SigmoidLayer<Dtype>::Backward_gpu(
    const vector<Blob<Dtype>*>& top,
    const vector<bool>& propagate_down,
    const vector<Blob<Dtype>*>& bottom) {
  if (propagate_down[0]) {
    ...
    const int count = (bottom)[0]->count();
    BOOL_CHECK(
        caffe::OpenCL::clSigmoidLayerBackward(count, top_diff,
