      ...
      for (int c = 0; c < layer->channels_; ++c) {
        for (int ph = 0; ph < layer->pooled_height_; ++ph) {
          for (int pw = 0; pw < layer->pooled_width_; ++pw) {
            int hstart = ph * layer->stride_h_ - layer->pad_h_;
            int wstart = pw * layer->stride_w_ - layer->pad_w_;
            int hend = min(hstart + layer->kernel_h_, layer->height_ + layer->pad_h_);
            int wend = min(wstart + layer->kernel_w_, layer->width_ + layer->pad_w_);
...
void PoolingCodeGeneratorForward<float>::Create_callback(PoolingLayer<float>* layer)
{
  ...
  { // AVX2 optimized version.
    ...
    if(layer->pad_h_ == 0 &&
       layer->pad_w_ == 0 &&
       (layer->pooled_height_-1) * layer->stride_h_ + layer->kernel_h_ == layer->height_ &&
       (layer->pooled_width_-1) * layer->stride_w_ + layer->kernel_w_ == layer->width_)
    ...
    int num_stack_qwords = 0;
    const Address& stack_bottom_data_ptr = qword[rbp - (++num_stack_qwords * 8)];
    const Address& stack_top_data_ptr    = qword[rbp - (++num_stack_qwords * 8)];
    const Address& stack_top_count       = qword[rbp - (++num_stack_qwords * 8)];
    const Address& stack_batch_start     = qword[rbp - (++num_stack_qwords * 8)];
    const Address& stack_batch_end       = qword[rbp - (++num_stack_qwords * 8)];
    const Address& stack_top_mask_ptr    = qword[rbp - (++num_stack_qwords * 8)];
    const Address& stack_batch_cnt       = qword[rbp - (++num_stack_qwords * 8)];
    const Address& stack_channel_cnt     = qword[rbp - (++num_stack_qwords * 8)];
    const Address& stack_out_w_cnt       = qword[rbp - (++num_stack_qwords * 8)];
    const Address& stack_out_h_cnt       = qword[rbp - (++num_stack_qwords * 8)];
    const Address& stack_pool_index      = qword[rbp - (++num_stack_qwords * 8)];
    const Address& stack_top_mask_orig   = qword[rbp - (++num_stack_qwords * 8)];
    const Address& stack_top_orig        = qword[rbp - (++num_stack_qwords * 8)];
    const Address& stack_bottom_orig     = qword[rbp - (++num_stack_qwords * 8)];
    const Address& stack_min_float       = dword[rbp - (++num_stack_qwords * 8)];
    const Address& stack_wstart0         = qword[rbp - (++num_stack_qwords * 8)];
    const Address& stack_index0          = qword[rbp - (++num_stack_qwords * 8)];
    const Address& stack_true_height     = qword[rbp - (++num_stack_qwords * 8)];
    const Address& stack_true_width      = qword[rbp - (++num_stack_qwords * 8)];
    ...
    // Move register arguments to the stack, we gonna need the registers for other purposes.
    ...
    // Further arguments on stack (layer_ptr and use_top_mask) are ingored in this implementation.
      ...
      jae("channel_loop_end", T_NEAR);
        ...
        jae("out_h_loop_end", T_NEAR);
          ...
          if(optimal_version)
          {
            ...
            jae("out_w_loop_end", T_NEAR);
              ...
              if(param.pooling_param().pool() == PoolingParameter_PoolMethod_MAX)
              {
                ...
                movaps(xmm0, xmm15);
                for(int kernel_h = 0; kernel_h < layer->kernel_h_; ++kernel_h)
                {
                  for(int kernel_w = 0; kernel_w < layer->kernel_w_; ++kernel_w)
                  {
                    ucomiss(xmm0, ptr[reg_scratch5 + kernel_w*sizeof(float) + kernel_h*layer->width_*sizeof(float)]);
                    maxss(xmm0, ptr[reg_scratch5 + kernel_w*sizeof(float) + kernel_h*layer->width_*sizeof(float)]);
              ...
              else if(param.pooling_param().pool() == PoolingParameter_PoolMethod_AVE)
              {
                xorps(xmm0, xmm0);
                for(int kernel_h = 0; kernel_h < layer->kernel_h_; ++kernel_h)
                  for(int kernel_w = 0; kernel_w < layer->kernel_w_; ++kernel_w)
                    addss(xmm0, ptr[reg_scratch5 + kernel_w*sizeof(float) + kernel_h*layer->width_*sizeof(float)]);
          ...
          else
          {
            ...
            // int hend = min(hstart, height_ - kernel_h_) + kernel_h_; stored in reg_scratch6
            mov(reg_scratch0, reg_scratch3);
            ...
            jae("out_w_loop_end", T_NEAR);
              ...
              // const int effective_ptr = (index)*4 + input_ptr; stored in reg_scratch4
              ...
              // We are reusing mulparam reg here (rax) but we won't be MULing inside.
...
/*
template <>
void PoolingCodeGeneratorBackward<float>::Create_callback(PoolingLayer<float>* layer)
{
  using namespace ::Xbyak;
  util::Cpu Current_cpu;
  if(Current_cpu.has(util::Cpu::tAVX2))
