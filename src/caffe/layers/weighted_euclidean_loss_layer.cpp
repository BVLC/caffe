...
void LossLayer<Dtype>::LayerSetUp(
  scalar_ = this->layer_param_.weighted_euclidean_loss_layer_param().positive_weight();
}
template <typename Dtype>
...
void WeightedEuclideanLossLayer<Dtype>::Forward_cpu(
  ...
  caffe_scal(count, scalar_, bottom[1]->mutable_cpu_data());
  caffe_cpu_mul(
      count,
      bottom[1]->cpu_data(),
      diff_.cpu_data(),
