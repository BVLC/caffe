...
struct ReusingShufflingDataCache : RemoteDataLayer<Dtype>::RemoteDataQueue {
  ...
  ReusingShufflingDataCache(RemoteDataParameter param)
    : read(0)
    , batch_size(0)
  ...
  virtual bool add_data(DataMsg& update) {
...

    {
      ...
      LOG(INFO) << "added data " << read << "/" << cache.size() << " " << read_all;
    }
...
struct SimpleDataCache : RemoteDataLayer<Dtype>::RemoteDataQueue {
  ...
  SimpleDataCache(RemoteDataParameter param)
    : cache_size(param.cache_size()) {
...
using namespace caffe::internode;
...
template <typename Dtype>
void RemoteDataLayer<Dtype>::DataLayerSetUp(const vector<Blob<Dtype>*>& bottom,
                                            const vector<Blob<Dtype>*>& top) {
