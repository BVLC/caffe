...
void hdf5_load_description(hid_t file_id, vector<int>* description) {
  ...
  CHECK_EQ(description->data()[0], dim-1);
}
...
void check_data(const int*data, const vector<int> & description,
                int num) {
  int numcategories = description[0];
  ...
  for (int j = 0; j < num; ++j)
    for (int i = 0; i < numcategories; ++i) {
      ...
      CHECK_GE(description[i+1], data[j*numcategories+i]);
    }
}
...
void hdf5_load_int_data(hid_t file_id, Blob<int>* blob,
  ...
  status= H5LTget_dataset_ndims(file_id, dataset_name_, &ndims);
  CHECK_GE(status, 0) << "Failed to get dataset ndims for " <<
    dataset_name_;
  ...
  CHECK_GE(status, 0) << "Failed to get dataset info for " <<
    dataset_name_;
  CHECK_EQ(class_, H5T_INTEGER) << "Expected integer data";
...
template <typename Dtype>
void
HDF5CategoricalDLayer<Dtype>::LoadHDF5FileCategoricalData(
                                const char* filename) {
...
HDF5CategoricalDLayer<Dtype>::setcategoriesvalues(const int * input,
  ...
  for (int i = 0; i < numcategories; ++i)
    if (input[i] > -1)
      onehotencodedvector[accumulatedvalues_[i]+input[i]] = (Dtype)1.0;
}
