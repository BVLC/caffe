...
void ReLULayer<Dtype>::Forward_cpu(
...
template<typename Dtype>
void ReLULayer<Dtype>::Backward_cpu(
    const vector<Blob<Dtype>*>& top,
...
template bool clReLULayerForward<float>(
    const int count,
    const float* bottom_data,
    float* top_data,
    float negative_slope);
template bool clReLULayerForward<double>(
    const int count,
...
template bool clReLULayerBackward<float>(
    const int count,
    const float* top_diff,
    const float* bottom_data,
    float* bottom_diff,
    float negative_slope);
template bool clReLULayerBackward<double>(
    const int count,
...
void ReLULayer<Dtype>::Forward_gpu(
  ...
  TIME("caffe::OpenCL::clReLULayerForward()", {
      BOOL_CHECK(caffe::OpenCL::clReLULayerForward(count, bottom_data,
                                                   top_data, negative_slope) );
  });
}
...
void ReLULayer<Dtype>::Backward_gpu(
  ...
  if (propagate_down[0]) {
    ...
    TIME("caffe::OpenCL::clReLULayerBackward()", {
        BOOL_CHECK(caffe::OpenCL::clReLULayerBackward(count, top_diff,
