...
template<typename Dtype>
void MVNLayer<Dtype>::Forward_cpu(
    const vector<Blob<Dtype>*>& bottom,
    const vector<Blob<Dtype>*>& top) {
  ...
  if (this->layer_param_.mvn_param().normalize_variance()) {
    ...
    caffe_powx(bottom[0]->count(), bottom_data,
               Dtype(2), temp_.mutable_cpu_data());
...
template<typename Dtype>
void MVNLayer<Dtype>::Backward_cpu(
    const vector<Blob<Dtype>*>& top,
  ...
  if (this->layer_param_.mvn_param().normalize_variance()) {
    ...
    caffe_powx(mean_.count(), mean_.cpu_data(),
               Dtype(2), temp_.mutable_cpu_data());  // (EX)^2
    caffe_sub(mean_.count(), variance_.cpu_data(), temp_.cpu_data(),
              variance_.mutable_cpu_data());  // variance
...
template<typename T> bool clMVNLayerForwardResidual(
    const T* bottom_data,
    const int bottom_data_height,
    const int bottom_data_width,
    const T* sum_multiplier,
    const int sum_multiplier_width,
    const T* mean,
    const int mean_width,
    const T* variance,
    const int variance_width,
    const T eps,
    T* top_data,
    const int top_data_height,
    const int top_data_width) {
  ...
  int dim = 1;
  size_t global[1] = { CAFFE_GET_GLOBAL_WORKITEMS(bottom_data_width
      * bottom_data_height,
  OPENCL_LOCAL_SIZE) };
  size_t local[1] = { CAFFE_GET_LOCAL_WORKITEMS(bottom_data_width
      * bottom_data_height,
  OPENCL_LOCAL_SIZE) };
...
template bool clMVNLayerForwardResidual<float>(
    ...
    const int top_data_height,
    const int top_data_width);
template bool clMVNLayerForwardResidual<double>(
...
template<typename T> bool clMVNLayerForwardMV2(
    const T* data2D,
    const int data2D_height,
    const int data2D_width,
    const T* data1D,
    const int data1D_length,
    T* linear_term,
    T* quadratic_term) {
  OpenCLDevice& current_device =
      OpenCLManager::CurrentPlatform()->CurrentDevice();
...
template bool clMVNLayerForwardMV2<float>(
    const float* data2D,
    const int data2D_height,
    const int data2D_width,
    const float* data1D,
...
template bool clMVNLayerForwardMV2<double>(
    const double* data2D,
    const int data2D_height,
    const int data2D_width,
    const double* data1D,
...
template<typename T> bool clMVNLayerForward(
    const T* data2D_in,
    const int data2D_in_height,
    const int data2D_in_width,
    const T* data1D_in,
    const int data1D_in_length,
    const T* linear_term,
    const int linear_term_length,
    const T* quadratic_term,
    const int quadratic_term_length,
    const T eps,
    T* data2D_out) {
  OpenCLDevice& current_device =
      OpenCLManager::CurrentPlatform()->CurrentDevice();
...
template bool clMVNLayerForward<float>(
    const float* data2D_in,
    const int data2D_in_height,
    const int data2D_in_width,
    const float* data1D_in,
    const int data1D_in_length,
    const float* linear_term,
    const int linear_term_length,
...
template bool clMVNLayerForward<double>(
    const double* data2D_in,
    const int data2D_in_height,
    const int data2D_in_width,
    const double* data1D_in,
    const int data1D_in_length,
...
template bool clMVNLayerBackwardMV2<float>(
    const float* data2D,
    const float* diff2D,
    const int data2D_height,
    const int data2D_width,
    ...
    float* linear_term,
    float* quadratic_term);
template bool clMVNLayerBackwardMV2<double>(
    const double* data2D,
    const double* diff2D,
    const int data2D_height,
    const int data2D_width,
...
template<typename T> bool clMVNLayerForward_perf(
    const T* A2D_top,
    const T* A2D_top_diff,
    const int top_height,
    const int top_width,
    const T* A2D_bottom,
    const T* A2D_bottom_diff,
    const int bottom_height,
    const int bottom_width,
    const T* A1D_sum_multiplier,
    const T* A1D_buffer,
    const int sum_multiplier_length,
    const T eps,
    T* data2D_out) {
  OpenCLDevice& current_device =
      OpenCLManager::CurrentPlatform()->CurrentDevice();
...
template bool clMVNLayerForward_perf<float>(
    const float* A2D_top,
    const float* A2D_top_diff,
    const int top_height,
    const int top_width,
    const float* A2D_bottom,
    const float* A2D_bottom_diff,
    const int bottom_height,
    const int bottom_width,
    const float* A1D_sum_multiplier,
    const float* A1D_buffer,
    const int sum_multiplier_length,
    const float eps,
    float* data2D_out);
template bool clMVNLayerForward_perf<double>(
    const double* A2D_top,
    const double* A2D_top_diff,
    const int top_height,
    const int top_width,
    const double* A2D_bottom,
    const double* A2D_bottom_diff,
    const int bottom_height,
    const int bottom_width,
    const double* A1D_sum_multiplier,
    const double* A1D_buffer,
    const int sum_multiplier_length,
    const double eps,
    double* data2D_out);
...
template<typename Dtype>
void MVNLayer<Dtype>::Forward_gpu(
    const vector<Blob<Dtype>*>& bottom,
    const vector<Blob<Dtype>*>& top) {
  ...
  if (this->layer_param_.mvn_param().normalize_variance()) {
    ...
    // this step computes the dot product between each part of bottom layer
    // and sum_multiplier and normalizes by number of pixels per part
    caffe_gpu_gemv<Dtype>(
        CblasNoTrans, num_parts, num_ppp, 1. / num_ppp,
        bottom_data,
    ...
    // 2D array [num_parts x num_ppp]
    // this step computes the dot product between each part of bottom layer
    // (squared) and sum_multiplier and normalizes by number of pixels per part
    caffe_gpu_gemv<Dtype>(
        CblasNoTrans,
        num_parts, num_ppp, 1. / num_ppp,
        temp_.gpu_data(),
        sum_multiplier_.gpu_data(),
        0.,
    ...
    /*
     ...
     caffe_gpu_powx(mean_.count(), mean_.gpu_data(),
                    Dtype(2), temp_.mutable_gpu_data());
    ...
    caffe::OpenCL::clMVNLayerForwardResidual(
        bottom_data, num_parts, num_ppp, sum_multiplier_.gpu_data(), num_ppp,
        mean_.gpu_data(), num_parts, variance_.gpu_data(), num_parts, eps,
        top_data, num_parts, num_ppp);

    /*
    caffe::OpenCL::clMVNLayerForwardMV2(bottom_data, num_parts, num_ppp,
                                        sum_multiplier_.gpu_data(), num_ppp,
                                        (Dtype*) mean_.mutable_gpu_data(),
                                        (Dtype*) variance_.mutable_gpu_data());
                                        */

    /*
    caffe::OpenCL::clMVNLayerForward(bottom_data, num_parts, num_ppp,
                                     sum_multiplier_.gpu_data(), num_ppp,
                                     (Dtype*) mean_.mutable_gpu_data(),
                                     num_parts,
                                     (Dtype*) variance_.mutable_gpu_data(),
                                     num_parts, eps, top_data);
                                     */
    /*
    caffe::OpenCL::clMVNLayerForward_perf(top_data, top_diff, num_parts,
                                          num_ppp, bottom_data, bottom_diff,
                                          num_parts, num_ppp,
                                          sum_multiplier_.gpu_data(),
                                          (Dtype*) temp_.mutable_gpu_data(),
                                          num_ppp, eps, top_data);
                                          */
  } else {
    /*
     caffe_gpu_gemv<Dtype>(
     ...
     caffe_gpu_gemm<Dtype>(
         CblasNoTrans, CblasNoTrans, num_parts, num_ppp, 1, -1.,
         mean_.gpu_data(),
         sum_multiplier_.gpu_data(),
         0.,
         temp_.mutable_gpu_data());
     caffe_gpu_add(temp_.count(), bottom_data, temp_.gpu_data(), top_data);
     */
    caffe::OpenCL::clMVNLayerForwardS2(bottom_data, num_parts, num_ppp,
                                       sum_multiplier_.gpu_data(), num_ppp,
                                       top_data);
  }
}
...
template<typename Dtype>
void MVNLayer<Dtype>::Backward_gpu(
    const vector<Blob<Dtype>*>& top,
    const vector<bool>& propagate_down,
    const vector<Blob<Dtype>*>& bottom) {
  const Dtype* top_diff = top[0]->gpu_diff();
  ...
  if (this->layer_param_.mvn_param().normalize_variance()) {
    /*
     ...
     caffe_gpu_sub(mean_.count(), variance_.gpu_data(),
                   temp_.gpu_data(), variance_.mutable_gpu_data());  // variance
    ...
    /*
     caffe::OpenCL::clMVNLayerBackwardMV2(top_data, top_diff, num, dim,
                                          sum_multiplier_.gpu_data(), dim,
                                          (Dtype*) mean_.mutable_gpu_data(),
                                          (Dtype*) variance_.mutable_gpu_data());

     caffe::OpenCL::clMVNLayerBackwardS1(top_data, top_diff, num, dim,
                                         sum_multiplier_.gpu_data(), dim,
                                         (Dtype*) mean_.mutable_gpu_data(),
                                         num,
                                         (Dtype*) variance_.mutable_gpu_data(),
     ...
     caffe::OpenCL::clMVNLayerForwardMV2(bottom_data, num, dim,
                                         sum_multiplier_.gpu_data(), dim,
                                         (Dtype*) mean_.mutable_gpu_data(),
                                         (Dtype*) variance_.mutable_gpu_data());

     caffe::OpenCL::clMVNLayerBackward(bottom_diff, num, dim,
                                       sum_multiplier_.gpu_data(), dim,
                                       (Dtype*) mean_.mutable_gpu_data(),
                                       num,
                                       (Dtype*) variance_.mutable_gpu_data(),
    ...
    caffe::OpenCL::clMVNLayerBackward_perf(top_data, top_diff, num, dim,
                                           bottom_data, bottom_diff, num, dim,
                                           sum_multiplier_.gpu_data(),
                                           reinterpret_cast<Dtype*>(temp_.mutable_gpu_data()),  // NOLINT(*)
                                           dim, eps, bottom_diff);
  } else {
    caffe_copy(temp_.count(), top_diff, bottom_diff);
  }
