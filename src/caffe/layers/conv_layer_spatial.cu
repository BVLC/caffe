...
#define dbgPrint(x)
  ...
  bool ConvolutionLayerSpatial<float>::generate_kernel(
    ...
    try {
      viennacl::ocl::program & program = submit_conv_spatial_program(ctx, kernel_name_, options);
      cl_ulong privateMemUsed;
      viennacl::ocl::kernel & kernel = program.get_kernel(kernel_name_);
      clGetKernelWorkGroupInfo(kernel.handle().get(), viennacl::ocl::current_device().id(),
  ...
  void ConvolutionLayerSpatial<float>::swizzleWeights(int swizzle_factor) {
    ...
    viennacl::ocl::kernel &oclk_copy_weight = program.get_kernel("copyWeightsSwizzled");
    cl_uint argIdx = 0;
    ...
    uint err = clEnqueueNDRangeKernel(ctx.get_queue().handle().get(),
         oclk_copy_weight.handle().get(), 3, NULL, global_work_size_Copy, NULL, 0, NULL,
        NULL);
  }
  ...
  void ConvolutionLayerSpatial<float>::pad_image(
    ...
    if (config->batched_execute) {
      for (int x = 0; x < imgNum; x++) {
        ...
        clEnqueueNDRangeKernel(ctx.get_queue().handle().get(), oclk_copy.handle().get(),
  ...
  bool ConvolutionLayerSpatial<float>::create_verification_kernel(
    ...
    try {
      viennacl::ocl::program & program = submit_conv_spatial_program(ctx, verification_kernel, options);
    } catch (std::exception& e)
    {
      dbgPrint(
    ...
    for (int n = 0; n < numImages; ++n) {
      for (int g = 0; g < group_; ++g) {
        ...
        if (config->use_null_local) {
          err = clEnqueueNDRangeKernel(ctx.get_queue().handle().get(), kernel.handle().get(), 3,
  ...
  bool ConvolutionLayerSpatial<float>::verify_result(
    ...
    viennacl::ocl::handle<cl_mem> verifcationResult = ctx.create_memory(CL_MEM_USE_HOST_PTR, sizeof(uint), &verificationFail);
    ...
    for (int n = 0; n < numImages; ++n) {
      for (int g = 0; g < group_; ++g) {
        ...
        size_t global_work_sizeB[3] = {(size_t)output_w_, (size_t)output_h_, (size_t)M_};
        err = clEnqueueNDRangeKernel(ctx.get_queue().handle().get(), kernel.handle().get(), 3,
            NULL, global_work_sizeB, NULL, 0, NULL, NULL);

        viennacl::backend::finish();
        clEnqueueMapBuffer(ctx.get_queue().handle().get(), verifcationResult, true,
  ...
  bool ConvolutionLayerSpatial<float>::setup_IDLF(
    ...
    viennacl::ocl::kernel & kernel = program.get_kernel(kernel_name_);
    cl_int err = clGetKernelWorkGroupInfo(kernel.handle().get(), viennacl::ocl::current_device().id(),
  ...
  void ConvolutionLayerSpatial<float>::setup_convolution(
    ...
    for (int x = 0; x < kernelQueue.size(); x++) {
      if (x != kernel_index_)
        ...
        viennacl::ocl::current_context().delete_program(kernelQueue[x]->kernelName);
    }
  ...
  void ConvolutionLayerSpatial<float>::Forward_gpu(
    ...
    for (int i = 0; i < bottom.size(); ++i) {
      ...
      if (kernelQueue[kernel_index_]->batched_execute)
        batched_convolve(bottom, top, i, num_,
            kernelQueue[kernel_index_]);
      else
        convolve(bottom, top, i, num_,
            kernelQueue[kernel_index_]);
  ...
  void ConvolutionLayerSpatial<float>::Backward_gpu(
    ...
    if (this->param_propagate_down_[0]) {
      ...
      greentea_gpu_set<float>(this->device_->id(), this->blobs_[0]->count(), 0.f, (cl_mem)weight_diff, 0.f);
    }
    float* bias_diff = NULL;
    if (bias_term_ && this->param_propagate_down_[1]) {
      bias_diff = this->blobs_[1]->mutable_gpu_diff();
      greentea_gpu_set(this->device_->id(), this->blobs_[1]->count(), 0.f, (cl_mem)bias_diff, 0.f);
    }
    ...
    for (int i = 0; i < top.size(); ++i) {
      ...
      if (bias_term_ && this->param_propagate_down_[1]) {
        ...
        for (int n = 0; n < num_; ++n) {
          greentea_gpu_gemv<float>(this->device_->id(), CblasNoTrans, num_output_, N_,
              1.f, (cl_mem)top_diff, n * this->top_dim_,
      ...
      if (this->param_propagate_down_[0] || propagate_down[i]) {
        ...
        for (int n = 0; n < num_; ++n) {
          ...
          greentea_im2col_gpu<float>(&program,
               &ctx, (cl_mem)bottom_data,
               n * this->bottom_dim_, channels_,
               height_, width_,
               kernel_h_, kernel_w_,
               pad_h_, pad_w_,
               stride_h_, stride_w_,
          ...
          if (this->param_propagate_down_[0]) {
            for (int g = 0; g < group_; ++g) {
              greentea_gpu_gemm<float>(this->device_->id(), CblasNoTrans, CblasTrans, M_, K_, N_,
                  1.f, (cl_mem)top_diff, n * this->top_dim_ + top_offset * g,
          ...
          if (propagate_down[i]) {
            ...
            for (int g = 0; g < group_; ++g) {
              greentea_gpu_gemm<float>(this->device_->id(), CblasTrans, CblasNoTrans, K_, N_, M_,
                  1.f, (cl_mem)weight, weight_offset * g,
                  (cl_mem)top_diff, n * this->top_dim_ + top_offset * g,
                  0.f, (cl_mem)col_diff, col_offset * g);
            }
            // col2im back to the data

            greentea_col2im_gpu<float>(&program,
                &ctx, (cl_mem)col_diff,
                0, channels_,
                height_, width_,
                kernel_h_, kernel_w_,
                ...
                stride_h_, stride_w_,
                1, 1,
                (cl_mem)bottom_diff, n * this->bottom_dim_);
