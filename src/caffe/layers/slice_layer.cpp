  ...
  if (slice_point_.size() != 0) {
    ...
    if (slice_dim_ == 0) {
      ...
      for (int i = 0; i < top.size(); ++i) {
        top[i]->Reshape(slices[i], channels_, height_, width_,this->device_context_);
        count_ += top[i]->count();
      }
    } else {
      slices.push_back(channels_ - prev);
      for (int i = 0; i < top.size(); ++i) {
        top[i]->Reshape(num_, slices[i], height_, width_,this->device_context_);
        count_ += top[i]->count();
      }
    }
  } else {
    ...
    for (int i = 0; i < top.size(); ++i) {
      top[i]->Reshape(num_, channels_, height_, width_,this->device_context_);
      count_ += top[i]->count();
    }
  }
