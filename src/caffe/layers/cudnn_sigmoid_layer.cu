...
template <typename Dtype>
void CuDNNSigmoidLayer<Dtype>::Forward_const_gpu(const vector<Blob<Dtype>*>& bottom,
  ...
  if (!activ_desc_ptr_.get()) {
    ...
    cudnn::createActivationDescriptor<Dtype>(activ_desc_ptr_.get(), CUDNN_ACTIVATION_SIGMOID);
  }
...
INSTANTIATE_LAYER_GPU_FUNCS_CONST(CuDNNSigmoidLayer);
