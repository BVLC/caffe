		...
		const bool propagate_down, vector<Blob<Dtype>*>* bottom) {
			...
			if (group_>1){
				...
				for (int n = 0; n<num_; n+=mem_group_size){
					...
					/*for (int i_batch = 0; i_batch<this_mem_group_size;i_batch++){
						im2col_gpu(bottom_data + (*bottom)[0]->offset(n+i_batch), channels_, height_,
						width_, kernel_size_, pad_, stride_, col_data+i_batch*col_offset);
					}*/
					...
					cu_im2mat_gpu(top_diff + top[0]->offset(n),1, M_, N_,
									col_diff,
									this_mem_group_size);
					...
					//Batch the weight diff calculation
					...
					if (propagate_down) {
						...
						// col2im back to the data
						/*for (int i_batch = 0; i_batch<this_mem_group_size;i_batch++){
							col2im_gpu(col_diff+col_offset*i_batch, channels_, height_, width_, kernel_size_, pad_,
							stride_, bottom_diff + (*bottom)[0]->offset(n+i_batch));
						}*/
						...
						bu_col2im_gpu_rot(col_data, channels_, height_, width_, kernel_size_, pad_,
							stride_, bottom_diff + (*bottom)[0]->offset(n), this_mem_group_size);
					}
				}
			}
