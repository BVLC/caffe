...
void LRNLayer<Dtype>::LayerSetUp(
  ...
  if (this->layer_param_.lrn_param().norm_region()
    ...
    pool_param.mutable_pooling_param()->set_pool(PoolingParameter_PoolMethod_AVE);  // NOLINT(*)
...
template<typename Dtype>
void LRNLayer<Dtype>::Forward_cpu(
    const vector<Blob<Dtype>*>& bottom,
    const vector<Blob<Dtype>*>& top) {
  switch (this->layer_param_.lrn_param().norm_region()) {
    case LRNParameter_NormRegion_ACROSS_CHANNELS:
...
template<typename Dtype>
void LRNLayer<Dtype>::Backward_cpu(
    const vector<Blob<Dtype>*>& top,
...
void LRNLayer<Dtype>::CrossChannelBackward_cpu(
  ...
  for (int n = 0; n < num_; ++n) {
    ...
    for (int c = 0; c < size_ - 1; ++c) {
      caffe_axpy<Dtype>(height_ * width_, 1., padded_ratio_data
...
void LRNLayer<Dtype>::WithinChannelBackward(
  ...
  if (propagate_down[0]) {
    ...
    square_layer_->Backward(square_top_vec_, propagate_down, square_bottom_vec_);  // NOLINT(*)
...
template bool clLRNFillScale<float>(
    const int nthreads,
    const float* in,
    const int num,
    const int channels,
    ...
    const float k,
    float* scale);
template bool clLRNFillScale<double>(
    const int nthreads,
    const double* in,
    const int num,
    const int channels,
...
template bool clLRNComputeOutput<float>(
    const int nthreads,
    const float* in,
    const float* scale,
    const float negative_beta,
    float* out);
template bool clLRNComputeOutput<double>(
    const int nthreads,
    const double* in,
    const double* scale,
...
template bool clLRNComputeDiff<float>(
    const int nthreads,
    ...
    float* bottom_diff);
template bool clLRNComputeDiff<double>(
    const int nthreads,
...
template<typename Dtype>
void LRNLayer<Dtype>::Forward_gpu(
    const vector<Blob<Dtype>*>& bottom,
    const vector<Blob<Dtype>*>& top) {
  switch (this->layer_param_.lrn_param().norm_region()) {
    case LRNParameter_NormRegion_ACROSS_CHANNELS:
...
void LRNLayer<Dtype>::CrossChannelForward_gpu(
  ...
  n_threads = bottom[0]->count();
  BOOL_CHECK(
      caffe::OpenCL::clLRNComputeOutput(
          n_threads, bottom_data, scale_data, -beta_, top_data));
}

template<typename Dtype>
void LRNLayer<Dtype>::Backward_gpu(
    const vector<Blob<Dtype>*>& top,
    const vector<bool>& propagate_down,
    const vector<Blob<Dtype>*>& bottom) {
  switch (this->layer_param_.lrn_param().norm_region()) {
...
void LRNLayer<Dtype>::CrossChannelBackward_gpu(
  ...
  BOOL_CHECK(
      caffe::OpenCL::clLRNComputeDiff(
          ...
          scale_.gpu_data(),
          top[0]->gpu_diff(),
          num_, channels_, height_, width_, size_, -beta_,
          Dtype(2.* alpha_ * beta_ / size_), bottom[0]->mutable_gpu_diff()));
}
