...
void ThresholdLayer<Dtype>::LayerSetUp(
...
template<typename Dtype>
void ThresholdLayer<Dtype>::Forward_cpu(
    const vector<Blob<Dtype>*>& bottom,
    const vector<Blob<Dtype>*>& top) {
...
template bool clThresholdForward<float>(
    const int n,
    const float threshold,
    const float* in,
    float* out);
template bool clThresholdForward<double>(
    const int n,
...
template<typename Dtype>
void ThresholdLayer<Dtype>::Forward_gpu(
    const vector<Blob<Dtype>*>& bottom,
    const vector<Blob<Dtype>*>& top) {
  ...
  /*
   ...
   ThresholdForward<Dtype> << <CAFFE_GET_BLOCKS(count), CAFFE_CUDA_NUM_THREADS>>>(count, threshold_, bottom_data, top_data);  // NOLINT(*)
   CUDA_POST_KERNEL_CHECK;
   */
  BOOL_CHECK(
      caffe::OpenCL::clThresholdForward(
