 ...
 * append the layer to the last full connection layer in the train_val.prototxt, like how you do with "Accuracy" layer.
...
namespace caffe {
    ...
    void ConfusionMatrixLayer<Dtype>::LayerSetUp(
        ...
        label_axis_ =
                bottom[0]->CanonicalAxisIndex(this->layer_param_.confusion_matrix_param().axis());
    ...
    void ConfusionMatrixLayer<Dtype>::Forward_cpu(const vector<Blob<Dtype> *> &bottom,
        ...
        for (int i = 0; i < outer_num_; ++i) {
            for (int j = 0; j < inner_num_; ++j) {
                ...
                buffer_data[label_value * num_labels + bottom_data_vector[0].second] += Dtype(1);
            }
        }
    }
    ...
    template<typename Dtype>
    void ConfusionMatrixLayer<Dtype>::Print_confusion_matrix(bool reset) {
        Dtype* buffer_data = nums_buffer_.mutable_cpu_data();
        stringstream ss;
        ss << "\n\nConfusion Matrix";
        ...
        for (int i = 0; i < num_labels; i++) {
            ...
            ss << "| " << std::fixed << std::setprecision(2) << 100.f * buffer_data[i * num_labels + i] / sum_i << "%\n";
        }
        int total_samples = 0;
        for (int i = 0; i < num_labels; i++) {
            total_samples += sum_j[i];
            ss << std::fixed << std::setprecision(2) << 100.f * buffer_data[i * num_labels + i] / sum_j[i] << "%\t";
        }
        ss << "| " << total_samples << "\n\n";
