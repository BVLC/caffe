...
namespace caffe {
  ...
  void ConfusionMatrixLayer<Dtype>::LayerSetUp(
    ...
    if (has_ignore_label_) {
      ignore_label_ =
        this->layer_param_.confusion_matrix_param().ignore_label();
    }
    ...
    caffe_set(num_labels * num_labels, Dtype(0),
  ...
  template<typename Dtype>
  void
  ConfusionMatrixLayer<Dtype>::Forward_cpu(const vector<Blob<Dtype> *> &bottom,
