...
void BaseConvolutionLayer<Dtype>::LayerSetUp(
  ...
  if (this->blobs_.size() > 0) {
    ...
    this->blobs_[0].reset(new Blob<Dtype>(
            conv_out_channels_,
            conv_in_channels_ / group_, kernel_h_, kernel_w_));
...
template<typename Dtype>
void BaseConvolutionLayer<Dtype>::forward_cpu_bias(
    Dtype* output,
    const Dtype* bias) {
  caffe_cpu_gemm<Dtype>(
      CblasNoTrans, CblasNoTrans,
...
template<typename Dtype>
void BaseConvolutionLayer<Dtype>::backward_cpu_gemm(
    const Dtype* output,
    const Dtype* weights,
    Dtype* input) {
...
template<typename Dtype>
void BaseConvolutionLayer<Dtype>::weight_cpu_gemm(
    const Dtype* input,
    const Dtype* output,
    Dtype* weights) {
  const Dtype* col_buff = input;
...
template<typename Dtype>
void BaseConvolutionLayer<Dtype>::backward_cpu_bias(
    Dtype* bias,
    const Dtype* input) {
...
template<typename Dtype>
void BaseConvolutionLayer<Dtype>::forward_gpu_bias(
    Dtype* output,
    const Dtype* bias) {
  ...
  for (int g = 0; g < group_; ++g) {
    ...
    TIME("backward_gpu_gemm()->caffe_gpu_gemm()", {
      /* call with offsets
       ...
       conv_out_channels_ / group_,
       (Dtype)1.,
       reinterpret_cast<const Dtype*>(weights) + weight_offset_ * g,
       reinterpret_cast<const Dtype*>(output) + output_offset_ * g,
       (Dtype)0.,
       reinterpret_cast<Dtype*>(col_buff) + col_offset_ * g);
       */
      caffe_gpu_gemm<Dtype>(
          CblasTrans, CblasNoTrans,
          kernel_dim_ / group_,
  ...
  for (int g = 0; g < group_; ++g) {
    ...
    TIME("weight_gpu_gemm()->caffe_gpu_gemm()", {
      /* call with offsets
       ...
       (Dtype)1.,
       reinterpret_cast<const Dtype*>(output) + output_offset_ * g,
       reinterpret_cast<const Dtype*>(col_buff) + col_offset_ * g,
       (Dtype)1.,
       reinterpret_cast<Dtype*>(weights) + weight_offset_ * g);
       */
      caffe_gpu_gemm<Dtype>(
          CblasNoTrans, CblasTrans,
          conv_out_channels_ / group_,
...
void BaseConvolutionLayer<Dtype>::forward_gpu_gemm(
  ...
  // TIMENOSYNC("forward_gpu_gemm()->im2col", {

  if (!is_1x1_) {
    if (!skip_im2col) {

      Dtype* buf;
      TIME("forward_gpu_gemm()->col_buffer_.mutable_gpu_data()", {
          buf = col_buffer_.mutable_gpu_data();
      });

      //TIMENOSYNC("forward_gpu_gemm()->conv_im2col_gpu()", {
      // conv_im2col_gpu((const Dtype*) input + input_offset, col_buffer_.mutable_gpu_data()); // NOLINT(*)
      //conv_im2col_gpu(input, input_offset, col_buffer_.mutable_gpu_data(), 0);


      TIME("forward_gpu_gemm()->im2col_gpu()", {
  ...
  //TIME("2nd step()", {
      for (int g = 0; g < group_; ++g) {
        TIME("forward_gpu_gemm()->caffe_gpu_gemm("<<g<<")", {
          /* call with offsets
           caffe_gpu_gemm<Dtype>(
           CblasNoTrans,
           CblasNoTrans,
          ...
          caffe_gpu_gemm<Dtype>(
              ...
              M, N, K,
              (Dtype)1.,
              weights, weights_offset + weight_offset_ * g,
...
void BaseConvolutionLayer<Dtype>::weight_gpu_gemm(
  ...
  for (int g = 0; g < group_; ++g) {
    TIME("weight_gpu_gemm()->caffe_gpu_gemm("<<g<<")", {
      /* old code commented out
       ...
       (Dtype)1.,
       output, output_offset + output_offset_ * g ,
       col_buff, col_buff_offset + col_offset_ * g,
       (Dtype)1.,
       weights, weights_offset + weight_offset_ * g);
       } else {
       */
