...
void ConvolutionLayer<Dtype>::fft_setup() {
  ...
  DftiSetValue(ifft_handle_,DFTI_CONJUGATE_EVEN_STORAGE, DFTI_COMPLEX_COMPLEX);
...
//  fprintf(stderr,"fft: num_threads = %d tid= %d\n ", num_of_threads_, tid );
  ...
  for (int c=0; c<channels_; c++){
    ...
    status = DftiComputeForward(fft_handle_,
	...
	for (int h_out = 0; h_out < height_out_; h_out++){
	  for (int w_out = 0; w_out < width_out_; w_out++){
	    ...
	    if ( (h < fft_height_) && (w < fft_width_))
	      map_out[h_out*width_out_ + w_out] = (Dtype) map_out_real[h*fft_width_ + w];
...
//-----------------------------------------------------------------------------
// see
// https://software.intel.com/sites/products/documentation/hpc/mkl/mklman/GUID-F4B8BEC6-70DD-4AB9-BBBB-DE95EBB67FA8.htm
//-----------------------------------------------------------------------------
