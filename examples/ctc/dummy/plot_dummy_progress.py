...
def softmax(data):
    m = np.max(data, axis=2)
    edata = np.exp(data - np.reshape(np.repeat(m, data.shape[2], axis=1), data.shape))
    s = np.sum(edata, axis=2)
    return edata / np.reshape(np.repeat(s, data.shape[2], axis=1), data.shape)
