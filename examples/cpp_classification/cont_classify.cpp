  ...
  while(1) {
    for (auto&& entry: boost::make_iterator_range(boost::filesystem::directory_iterator(folder), {})) {
      ...
      if (is_regular_file(p) && p.extension() == ".jpg") {
        std::time_t ts = boost::filesystem::last_write_time(p); 
        if (ts > latest_tm) {
          latest = p;
          latest_tm = ts;
        }
    ...
    std::cout << "---------- Prediction for "
              << latest.string() << " ----------" << std::endl;
     
    cv::Mat img = cv::imread(latest.string(), -1);
    if (img.empty()) {
      std::cout << "Unable to decode image " << latest.string() << std::endl;
      sleep(1);
      continue;
    }
    ...
    std::ofstream myfile;
    myfile.open("/var/tmp/topics.csv");
    for (size_t i = 0; i < predictions.size(); ++i) {
      Prediction p = predictions[i];
      std::cout << std::fixed << std::setprecision(4) << p.second << " - \""
                << p.first << "\"" << std::endl;
      myfile << p.first << '\t' << std::fixed << std::setprecision(4) << p.second << std::endl;
    }
    myfile.close();
  }
}
